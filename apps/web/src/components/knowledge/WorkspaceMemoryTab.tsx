...
import type { MemoryEntryRowData, MemoryKind } from './types';

export function WorkspaceMemoryTab() {
  const toast = useToast();
  const [entries, setEntries] = useState<MemoryEntryRowData[]>([]);
  ...
  async function refresh() {
    setLoading(true);
    try {
      const data = await api<{ memory: MemoryEntryRowData[] }>('/v1/memory?limit=100');
    ...
    <div className="space-y-4">
      ...
      {loading ? <Skeleton height={220} /> : filtered.length === 0 ? (
        <EmptyState
          ...
          body="Add facts, rules, and preferences that every app and agent in this workspace can use as shared context."
        />
      ) : (
        <div className="space-y-2">{filtered.map((entry) => <MemoryEntryRow key={entry.id} entry={entry} onArchive={(id) => void archive(id)} />)}</div>
      )}
    </div>
  );
}
