  ...
  return (
    <div className="space-y-2 px-3 pb-2">
      {approvals.slice(0, 4).map((approval) => (
        <div
          key={approval.id}
          className="rounded-card border border-amber-400/30 bg-amber-400/[0.06] p-3"
        >
          <div className="flex items-center gap-2">
            <ShieldCheck size={14} className="shrink-0 text-amber-400" />
            <span className="truncate text-[12px] font-medium text-text-primary">
              {approval.workflowName ?? 'Approval needed'}
            </span>
            <span className="ml-auto shrink-0 text-[10px] uppercase tracking-wide text-amber-400/80">
              awaiting you
            </span>
          </div>
          ...
          <div className="mt-2 flex gap-2">
            <button
              ...
              onClick={() => void resolve(approval, 'approve')}
            ...
            <button
              type="button"
              disabled={busy[approval.id]}
              onClick={() => void resolve(approval, 'reject')}
