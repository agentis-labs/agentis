...
export function BrainDetailRail({ brain, node, appSlug }: RailProps) {
  ...
  const atomKind = typeof m.atomKind === 'string' ? m.atomKind : null;
  const adapterType = typeof m.adapterType === 'string' ? m.adapterType : null;
  const agentId = typeof m.agentId === 'string' ? m.agentId : null;
  const reinforceCount = typeof m.reinforceCount === 'number' ? m.reinforceCount : null;
  const workspaceGlobal = m.workspaceGlobal === true;
  const disputed = m.isDisputed === true;
  ...
  return (
    <div className="flex h-full w-80 shrink-0 flex-col border-l border-line bg-surface">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 text-[12px]">
        ...
        {atomKind && (
          <Group title="Collective Brain">
            <Stat label="Atom" value={atomKind} />
            {adapterType ? <Stat label="Adapter" value={adapterType} /> : null}
            {agentId ? <Stat label="Agent" value={agentId} mono /> : null}
            {reinforceCount ? <Stat label="Reinforced" value={`${reinforceCount}x`} /> : null}
            {workspaceGlobal ? <Stat label="Scope" value="workspace-global" /> : null}
            {disputed ? <Stat label="State" value="disputed" /> : null}
          </Group>
        )}
