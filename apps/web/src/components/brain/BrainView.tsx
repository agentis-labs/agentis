...
export function BrainView({ slug }: BrainViewProps) {
  ...
  useEffect(() => {
    setLoading(true);
    setSelectedId(null);
    const url = slug ? `/v1/apps/${slug}/brain` : `/v1/brain`;
    void api<BrainResponse>(url)
      .then((d) => setData(d))
...
function Stat({ label, value, tone }: { label: string; value: string | number; tone?: 'warn' }) {
  return (
    <div className="rounded-card border border-line bg-bg-base px-2.5 py-1.5">
      <div className={['text-[14px] font-semibold leading-tight', tone === 'warn' ? 'text-amber-300' : 'text-text-primary'].join(' ')}>
        {value}
      </div>
