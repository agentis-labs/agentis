...
export function BrainNodeCard({ node, selected, dim, onClick }: CardProps) {
  ...
  const adapterType = typeof node.metadata.adapterType === 'string' ? node.metadata.adapterType : null;
  const reinforceCount = typeof node.metadata.reinforceCount === 'number' ? node.metadata.reinforceCount : null;
    ...
    >
      ...
      {(node.confidence != null || node.trust != null || node.freshness) && (
        <div className="flex flex-wrap items-center gap-1.5 text-[9px] uppercase tracking-wider text-text-muted">
          ...
          {adapterType && <Pill>{adapterType}</Pill>}
          {reinforceCount && reinforceCount > 1 && <Pill>{reinforceCount}x</Pill>}
        </div>
      )}
    </button>
  );
