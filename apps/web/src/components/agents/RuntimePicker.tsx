...
export function RuntimePicker({
  ...
  const [detections, setDetections] = useState<HarnessDetectionResult[]>([]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setDetecting(true);
    void api<{ harnesses: HarnessDetectionResult[] }>('/v1/harness/detect')
      .then((result) => {
        if (!cancelled) setDetections(result.harnesses ?? []);
      })
      .catch(() => {
  ...
  const detectedAdapters = ADAPTERS.filter((adapter) => detectionByType.get(adapter.id)?.status === 'found');
      ...
      {editing ? (
        ...
        <div className="space-y-4">
          ...
          <HarnessGrid
            ...
            onAdapterChange={onAdapterChange}
          />
          {activeDetection && activeDetection.status !== 'found' && activeDetection.installCommand && (
            <div className="rounded-lg border border-line bg-surface-2 px-3 py-2 text-xs text-text-muted">
              <span className="text-text-primary">{activeAdapter.title}</span> installation: <code className="text-text-primary">{activeDetection.installCommand}</code>
            </div>
          )}
        </div>
      )}
      <AdapterConfigFields adapterType={adapterType} config={runtimeConfig} setConfig={setConfig} />
    </div>
  );
}
...
function HarnessGrid({
  ...
  return (
    <div>
      <div className="mb-2 text-[11px] font-medium uppercase tracking-wider text-text-muted">{title}</div>
      <div className="grid grid-cols-3 gap-2 xl:grid-cols-6">
        {adapters.map((adapter) => {
          ...
          return (
            <button
              key={`${title}-${adapter.id}`}
              type="button"
              onClick={() => onAdapterChange(adapter.id)}
              className={clsx(
                'relative flex h-24 min-w-0 flex-col items-center justify-center gap-2 rounded-lg border px-2 text-center transition',
            ...
            >
              ...
              {detection?.status === 'found' && <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />}
            </button>
          );
        })}
      </div>
...
function AdapterConfigFields({
  ...
  if (adapterType === 'hermes_agent') {
    return (
      <div className="grid gap-3 md:grid-cols-5">
        ...
        <Field label="Model"><input value={config.hermesModel} onChange={(event) => setConfig('hermesModel', event.target.value)} placeholder="Model override" className={inputCls} /></Field>
  ...
  if (adapterType === 'claude_code') {
    return (
      <div className="grid gap-3 md:grid-cols-5">
        ...
        <Field label="Model"><input value={config.claudeModel} onChange={(event) => setConfig('claudeModel', event.target.value)} placeholder="claude-opus-4-5" className={inputCls} /></Field>
  ...
  if (adapterType === 'codex') {
    return (
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <Field label="Model"><input value={config.codexModel} onChange={(event) => setConfig('codexModel', event.target.value)} placeholder="gpt-5.3-codex" className={inputCls} /></Field>
  ...
  if (adapterType === 'cursor') {
    return (
      <div className="grid gap-3 md:grid-cols-4">
        <Field label="Binary path"><input value={config.cursorBinaryPath} onChange={(event) => setConfig('cursorBinaryPath', event.target.value)} placeholder="cursor" className={inputCls} /></Field>
        ...
        <Field label="Model"><input value={config.cursorModel} onChange={(event) => setConfig('cursorModel', event.target.value)} placeholder="auto" className={inputCls} /></Field>
...
export function configToRuntimeConfig(adapterType: AdapterType, stored: Record<string, unknown>): RuntimeConfig {
  ...
  if (adapterType === 'openclaw') return { ...base, openclawGatewayId: stringOf(stored.gatewayId), openclawGatewayUrl: stringOf(stored.gatewayUrl), openclawDeviceTokenCredentialId: stringOf(stored.deviceTokenCredentialId), openclawAgentName: stringOf(stored.agentName), openclawSessionKeyStrategy: stringOf(stored.sessionKeyStrategy, DEFAULT_RUNTIME_CONFIG.openclawSessionKeyStrategy), openclawSessionKey: stringOf(stored.sessionKey), openclawTimeoutSec: stringOf(stored.timeoutSec, DEFAULT_RUNTIME_CONFIG.openclawTimeoutSec), openclawPayloadTemplate: jsonText(stored.payloadTemplate) };
  if (adapterType === 'hermes_agent') return { ...base, hermesBinaryPath: stringOf(stored.binaryPath), hermesCwd: stringOf(stored.cwd), hermesModel: stringOf(stored.model), hermesMaxTurns: stringOf(stored.maxTurns, DEFAULT_RUNTIME_CONFIG.hermesMaxTurns), hermesExtraArgs: arrayText(stored.extraArgs), hermesEnv: jsonText(stored.env), hermesTimeoutSec: stringOf(stored.timeoutSec), hermesGraceSec: stringOf(stored.graceSec) };
  if (adapterType === 'claude_code') return { ...base, claudeBinaryPath: stringOf(stored.binaryPath), claudeCwd: stringOf(stored.cwd), claudeModel: stringOf(stored.model), claudeMaxTurns: stringOf(stored.maxTurns, DEFAULT_RUNTIME_CONFIG.claudeMaxTurns), claudeAllowedTools: arrayText(stored.allowedTools), claudeExtraArgs: arrayText(stored.extraArgs), claudeEnv: jsonText(stored.env), claudeTimeoutSec: stringOf(stored.timeoutSec) };
  if (adapterType === 'codex') return { ...base, codexBinaryPath: stringOf(stored.binaryPath), codexCwd: stringOf(stored.cwd), codexModel: stringOf(stored.model, DEFAULT_RUNTIME_CONFIG.codexModel), codexMaxTurns: stringOf(stored.maxTurns, DEFAULT_RUNTIME_CONFIG.codexMaxTurns), codexReasoningEffort: stringOf(stored.modelReasoningEffort), codexFastMode: boolText(stored.fastMode, DEFAULT_RUNTIME_CONFIG.codexFastMode), codexBypassApprovalsAndSandbox: boolText(stored.dangerouslyBypassApprovalsAndSandbox, DEFAULT_RUNTIME_CONFIG.codexBypassApprovalsAndSandbox), codexExtraArgs: arrayText(stored.extraArgs), codexEnv: jsonText(stored.env), codexTimeoutSec: stringOf(stored.timeoutSec) };
  if (adapterType === 'cursor') return { ...base, cursorBinaryPath: stringOf(stored.binaryPath), cursorCwd: stringOf(stored.cwd), cursorModel: stringOf(stored.model, DEFAULT_RUNTIME_CONFIG.cursorModel), cursorExtraArgs: arrayText(stored.extraArgs), cursorEnv: jsonText(stored.env), cursorTimeoutSec: stringOf(stored.timeoutSec) };
...
export function runtimeConfigToAdapterConfig(adapterType: AdapterType, config: RuntimeConfig): Record<string, unknown> {
  if (adapterType === 'openclaw') return compact({ gatewayId: config.openclawGatewayId, gatewayUrl: config.openclawGatewayUrl, agentName: config.openclawAgentName, deviceTokenCredentialId: config.openclawDeviceTokenCredentialId, sessionKeyStrategy: config.openclawSessionKeyStrategy, sessionKey: config.openclawSessionKey, timeoutSec: positiveNumber(config.openclawTimeoutSec), payloadTemplate: jsonObject(config.openclawPayloadTemplate) });
  if (adapterType === 'hermes_agent') return compact({ binaryPath: config.hermesBinaryPath, cwd: config.hermesCwd, model: config.hermesModel, maxTurns: positiveNumber(config.hermesMaxTurns), extraArgs: splitArgs(config.hermesExtraArgs), env: jsonStringRecord(config.hermesEnv), timeoutSec: positiveNumber(config.hermesTimeoutSec), graceSec: positiveNumber(config.hermesGraceSec) });
  if (adapterType === 'claude_code') return compact({ binaryPath: config.claudeBinaryPath, cwd: config.claudeCwd, model: config.claudeModel, maxTurns: positiveNumber(config.claudeMaxTurns), allowedTools: splitCsv(config.claudeAllowedTools), extraArgs: splitArgs(config.claudeExtraArgs), env: jsonStringRecord(config.claudeEnv), timeoutSec: positiveNumber(config.claudeTimeoutSec) });
  ...
  if (adapterType === 'cursor') return compact({ binaryPath: config.cursorBinaryPath, cwd: config.cursorCwd, model: config.cursorModel, extraArgs: splitArgs(config.cursorExtraArgs), env: jsonStringRecord(config.cursorEnv), timeoutSec: positiveNumber(config.cursorTimeoutSec) });
...
export function runtimeLabelFor(adapterType: AdapterType, config: RuntimeConfig): string {
  if (adapterType === 'openclaw') return config.openclawAgentName || 'OpenClaw';
