...
export function DataOwnershipPanel() {
  ...
  useEffect(() => { load(); }, [load]);
  // The Brain is cared for continuously as you work â€” reflect that live.
  useRealtime(
  ...
  async function exportAll() {
    ...
    try {
      ...
      toast.success('Your data was downloaded', 'An open, re-importable copy â€” it never left your machine.');
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <p className="-mt-3 text-[13px] text-text-secondary">
        Everything Agentis remembers for you â€” owned by you, kept on your machine, yours to take or erase.
      </p>

      {/* Ownership hero â€” the emotional core */}
      <section className="rounded-card border border-line bg-surface-2 p-5">
        <div className="flex flex-wrap items-start justify-between gap-x-8 gap-y-4">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 max-w-md text-[13px] leading-relaxed text-text-secondary">
                ...
                you own â€” nothing in it is sent anywhere unless you send it.
              </p>
              <div className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-1 text-[12px] text-text-muted">
                <Database size={12} className="shrink-0" />
                <span className="truncate font-mono">{data?.storage.path ?? 'â€¦'}</span>
                {data && (
                  <>
          ...
          <div className="flex flex-col items-stretch gap-1.5">
            ...
            >
              <Download size={14} />
              {exporting ? 'Preparingâ€¦' : 'Download everything'}
            </button>
            <span className="text-center text-[11px] text-text-muted">Open format Â· re-importable</span>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <p className="mt-1.5 text-[13px] text-text-secondary">
          An agentâ€™s brain and full state belong to you. Change the runtime underneath â€” keep the brain.
        </p>
              ...
              >
                ...
                <div className="flex items-center gap-1.5 text-[11px] text-text-muted">
                  ...
                  <span className="truncate">
                    {runtimeLabel(agent.adapterType)}{agent.runtimeModel ? ` Â· ${agent.runtimeModel}` : ''}
                  </span>
                </div>
                <div className="flex items-center justify-between border-t border-line pt-2.5 text-[11px]">
                  <span className="text-text-secondary">
                    <span className="tabular-nums font-medium text-text-primary">{agent.memories}</span> {agent.memories === 1 ? 'memory' : 'memories'} Â· portable
                  </span>
  ...
  return (
    <div className="rounded-card border border-line bg-surface p-4">
      <div className="text-[24px] font-semibold tabular-nums text-text-primary">
        {value === undefined ? <span className="text-text-muted">â€”</span> : value.toLocaleString()}
      </div>
