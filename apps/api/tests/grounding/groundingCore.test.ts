/**
 * Grounding core loop — evidence ledger idempotency, claim formation gating,
...
import { EvidenceLedgerService } from '../../src/grounding/evidenceLedger.js';
import { GroundingSourceFabric } from '../../src/grounding/sourceFabric.js';
import { AgentisNativeSource } from '../../src/grounding/sources/agentisNativeSource.js';
import { ClaimService } from '../../src/grounding/claimService.js';
import { IdentityService } from '../../src/grounding/identityService.js';
import { GroundingContextComposer } from '../../src/grounding/contextComposer.js';
import { GroundingMigrationService } from '../../src/grounding/migrationService.js';
import { GroundingDiscoveryService } from '../../src/grounding/discovery.js';
import { GroundingExtractionService } from '../../src/grounding/extractionService.js';
import { GroundingModelService } from '../../src/grounding/modelService.js';
import { GroundingRuntime } from '../../src/grounding/groundingRuntime.js';
import type {
...
describe('Grounding core loop', () => {
  let ctx: TestContext;
  let ledger: EvidenceLedgerService;
  let fabric: GroundingSourceFabric;
  let claims: ClaimService;
  let identity: IdentityService;
  let composer: GroundingContextComposer;
  let migration: GroundingMigrationService;
  ...
  beforeEach(async () => {
    ...
    fabric = new GroundingSourceFabric({ db: ctx.db, logger: ctx.logger, ledger });
    fabric.register(new AgentisNativeSource(ctx.db));
    identity = new IdentityService({ db: ctx.db, logger: ctx.logger });
    composer = new GroundingContextComposer({ db: ctx.db, logger: ctx.logger });
    migration = new GroundingMigrationService({ db: ctx.db, logger: ctx.logger, claims });
  ...
  it('secrets are redacted before persistence and labeled', () => {
    const stripeKey = ['sk', 'live', 'abcdefghijklmnop1234'].join('_');
    const result = record('leak-1', `Deploy with api_key = "${stripeKey}" please.`);
    ...
    expect(normalized.content).not.toContain(stripeKey);
  ...
  it('contradicting claims create a conflict riding the knowledge_links dispute space', () => {
    ...
    expect(link.sourceKind).toBe('grounding_claim');
  ...
  it('verified-equal emails link deterministically; probabilistic stays in review', () => {
    ...
    const links = ctx.db.select().from(schema.groundingIdentityLinks)
      .where(eq(schema.groundingIdentityLinks.workspaceId, ctx.workspace.id)).all();
  ...
  it('agentis_native backfill ingests workspace activity idempotently; launch is idempotent', async () => {
    ...
    const discovery = new GroundingDiscoveryService({ db: ctx.db, logger: ctx.logger, fabric, composer });
  ...
  it('the disclosure validator blocks secret-shaped strings in outbound text', () => {
    ...
    const verdict = composer.validateDisclosure(ctx.workspace.id, `Here is the key: ${awsKey} for prod.`);
  ...
  it('runs the whole loop without manual claims: native activity becomes agent context', async () => {
    ...
    const extraction = new GroundingExtractionService({ db: ctx.db, logger: ctx.logger, claims, identity, migration });
    const model = new GroundingModelService({ db: ctx.db, logger: ctx.logger, claims });
    const discovery = new GroundingDiscoveryService({ db: ctx.db, logger: ctx.logger, fabric, composer });
    const runtime = new GroundingRuntime({ db: ctx.db, logger: ctx.logger, fabric, extraction, model, discovery });
  ...
  it('resolves the bearer token from an OAuth bundle AND a raw token credential', async () => {
    ...
    const vaultFabric = new GroundingSourceFabric({ db: ctx.db, logger: ctx.logger, ledger, vault: ctx.vault });
