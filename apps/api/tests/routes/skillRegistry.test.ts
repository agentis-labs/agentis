/**
 * /v1/skills/registry routes — bridge integration with mocked RegistryClient.
 */
...
import { buildSkillRegistryRoutes } from '../../src/routes/skillRegistry.js';
...
describe('/v1/skills/registry', () => {
  ...
  it('GET /status returns configured + breaker', async () => {
    ...
    const app = ctx.buildApp([{ path: '/v1/skills/registry', app: buildSkillRegistryRoutes({ db: ctx.db, auth: ctx.auth, registry: bridge, activity }) }]);
    const res = await app.request('/v1/skills/registry/status', { headers: ctx.authHeaders });
  ...
  it('GET /registry returns entries from the bridge', async () => {
    ...
    const app = ctx.buildApp([{ path: '/v1/skills/registry', app: buildSkillRegistryRoutes({ db: ctx.db, auth: ctx.auth, registry: bridge, activity }) }]);
    const res = await app.request('/v1/skills/registry', { headers: ctx.authHeaders });
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it('POST /install/:slug verifies SHA-256 and writes installed_registry_artifacts + activity row', async () => {
    ...
    const app = ctx.buildApp([{ path: '/v1/skills/registry', app: buildSkillRegistryRoutes({ db: ctx.db, auth: ctx.auth, registry: bridge, activity }) }]);
    const res = await app.request('/v1/skills/registry/install/demo', {
      method: 'POST',
    ...
    expect(events.some((e) => e.eventType === 'skill_registry.installed')).toBe(true);
  });
  ...
  it('POST /install/:slug rejects when SHA-256 does not match', async () => {
    ...
    const body = await res.json();
    expect(body.error?.code).toBe('SKILL_REGISTRY_HASH_MISMATCH');
  });

  it('POST /install/:slug blocks artifacts containing secrets', async () => {
    const bytes = Buffer.from('const k = "AKIAABCDEFGHIJKLMNOP";');
    const bridge = buildBridge({ bad: { entry: fakeEntry('bad', bytes.toString()), bytes } });
    const app = ctx.buildApp([{ path: '/v1/skills/registry', app: buildSkillRegistryRoutes({ db: ctx.db, auth: ctx.auth, registry: bridge, activity }) }]);
    const res = await app.request('/v1/skills/registry/install/bad', {
      method: 'POST',
      headers: { ...ctx.authHeaders, 'content-type': 'application/json' },
      body: JSON.stringify({ permissionsAcknowledged: true }),
    });
    expect(res.status).toBe(422);
    const body = await res.json();
    expect(body.error?.code).toBe('SKILL_REGISTRY_SCAN_BLOCKED');
  });
});
