/**
 * /v1/extensions/registry — anonymous third-party extension registry endpoints
 * (V1-SPEC §8).
 *
 ...
 *
 *   - EXTENSION_REGISTRY_UNAVAILABLE (503) when the client isn't configured
 ...
 *     `block`-severity finding throws EXTENSION_REGISTRY_SCAN_BLOCKED.
 ...
 *
 * Endpoint surface (mounted at /v1/extensions/registry):
...
export function buildExtensionRegistryRoutes(deps: {
  db: AgentisSqliteDb;
  auth: AuthService;
  ...
  app.post('/install/:slug', async (c) => {
    ...
    deps.activity.record({
      ...
      ambientId: ws.ambientId,
      userId: ws.user.id,
      eventType: 'extension_registry.installed',
      actorType: 'user',
      actorId: ws.user.id,
