...
export function buildMemoryRoutes(deps: MemoryRoutesDeps) {
  ...
  app.post('/working/:runId/compact', (c) => {
    const runId = c.req.param('runId');
    const summary = deps.memory.compactWorking(runId);
    return c.json({ summary });
  });
  ...
  app.get('/working/:runId/:namespace/:kind/:key', (c) => {
    ...
    const value = deps.memory.readWorking(runId, namespace, kind, key);
  ...
  app.put('/working/:runId/:namespace/:kind/:key', async (c) => {
    ...
    deps.memory.writeWorking(runId, namespace, kind, key, body.payload);
  ...
  app.get('/episodes', (c) => {
    ...
    const args: Parameters<typeof deps.episodes.list>[0] = {
    ...
    const episodes = deps.episodes.list(args);
  ...
  app.get('/episodes/:id', (c) => {
    ...
    const ep = deps.episodes.byId(ws.workspaceId, id);
  ...
  app.patch('/episodes/:id', async (c) => {
    ...
    const updated = deps.episodes.update(ws.workspaceId, id, body);
  ...
  app.delete('/episodes/:id', (c) => {
    const ws = getWorkspace(c);
    const id = c.req.param('id');
    const ok = deps.episodes.archive(ws.workspaceId, id);
  ...
  app.post('/episodes/:id/reinforce', async (c) => {
    ...
    const ep = deps.episodes.reinforce(ws.workspaceId, id, body);
    if (!ep) throw new AgentisError('RESOURCE_NOT_FOUND', `episode '${id}' not found`);
    return c.json({ episode: ep });
  });
  ...
  app.post('/episodes/search', async (c) => {
    ...
    const args: Parameters<typeof deps.episodes.searchEpisodes>[0] = {
      workspaceId: ws.workspaceId,
    };
    ...
    const hits = deps.episodes.searchEpisodes(args);
  ...
  app.post('/promotions/promote', async (c) => {
    ...
    const decisions = body.candidates.map((candidate) => {
      const args: Parameters<typeof deps.promotion.promoteCandidate>[0] = {
        workspaceId: ws.workspaceId,
      ...
      return deps.promotion.promoteCandidate(args);
    });
    const summary = {
  ...
  app.get('/promotions', (c) => {
    const ws = getWorkspace(c);
    const args: Parameters<typeof deps.promotion.listEvents>[0] = {
      workspaceId: ws.workspaceId,
    };
    ...
    const events = deps.promotion.listEvents(args);
  ...
  app.get('/baselines/:workflowId', (c) => {
    ...
    const latest = deps.rollingBaselines.latest(ws.workspaceId, workflowId);
  ...
  app.get('/baselines/:workflowId/history', (c) => {
    ...
    const limit = parseIntOr(c.req.query('limit'));
    const args: Parameters<typeof deps.rollingBaselines.history>[0] = {
      workspaceId: ws.workspaceId,
      workflowId,
      window,
    };
    if (limit) args.limit = limit;
    const history = deps.rollingBaselines.history(args);
  ...
  app.post('/baselines/:workflowId/capture', async (c) => {
    ...
    const snapshot = deps.rollingBaselines.capture({
      workspaceId: ws.workspaceId,
      workflowId,
      ...(body.appId !== undefined ? { appId: body.appId } : {}),
  ...
  app.post('/baselines/:workflowId/anomalies', async (c) => {
    ...
    const observed: Parameters<typeof deps.rollingBaselines.detectAnomalies>[1] = {
      workflowId,
    ...
    const anomalies = deps.rollingBaselines.detectAnomalies(ws.workspaceId, observed);
