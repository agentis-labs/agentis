/**
 * /v1/grounding — the Workspace Brain's organizational intelligence engine
 * (RFC §15). Engineering namespace; the UI surfaces this as the Brain
 * "Sources" tab — never as a "Grounding" page.
 *
...
import type { GroundingSourceFabric } from '../grounding/sourceFabric.js';
import type { EvidenceLedgerService } from '../grounding/evidenceLedger.js';
import type { ClaimService } from '../grounding/claimService.js';
import type { IdentityService } from '../grounding/identityService.js';
import type { GroundingModelService } from '../grounding/modelService.js';
import type { GroundingContextComposer } from '../grounding/contextComposer.js';
import type { GroundingMigrationService } from '../grounding/migrationService.js';
...
import { buildOrganizationalOverlay } from '../grounding/graphProjection.js';
import type { GroundingInvestigationService } from '../grounding/investigationService.js';
import { requireAuth } from '../middleware/auth.js';
import { requireWorkspace, getWorkspace } from '../middleware/workspace.js';

export interface GroundingRoutesDeps {
  db: AgentisSqliteDb;
  auth: AuthService;
  ...
  model: GroundingModelService;
  composer: GroundingContextComposer;
  migration: GroundingMigrationService;
  discovery: GroundingDiscoveryService;
  runtime: GroundingRuntime;
  investigations: GroundingInvestigationService;
}
 ...
 */
export function buildGroundingWebhookRoutes(deps: { fabric: GroundingSourceFabric }) {
  const app = new Hono();
...
export function buildGroundingRoutes(deps: GroundingRoutesDeps) {
  const app = new Hono();
  ...
  app.get('/graph', (c) => {
    const ws = getWorkspace(c);
    const ensured = deps.discovery.ensureInternalWorkspace({ workspaceId: ws.workspaceId, ownerUserId: ws.user.id });
    if (ensured.created || ensured.needsSync) {
      void deps.runtime.tickWorkspace(ws.workspaceId).catch(() => {});
    }
