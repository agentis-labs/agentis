 ...
 *
 * Owns the two agentic looping node kinds â€” `loop` (bounded fan-out over an
 ...
 *
 * This is the AGENT-COOPERATION-10X Â§Pillar 1 + COGNITIVE-LOOPING-RFC loop.
 */
import {
  AgentisError,
  REALTIME_EVENTS,
...
export class ConvergeLoopController {
  ...
  async dispatchLoop(
    ...
    // Resolve items array Ã¢â‚¬â€ accept either a `{{path}}` template or a raw dot path.
    let items: unknown;
    if (config.itemsExpression?.includes('{{')) {
      // Stringified pass Ã¢â‚¬â€ we need typed access, so read the path directly.
  ...
  async #runLoop(
    ...
    // on re-run â€” side effects fire at most once per iteration instead of the
    ...
    for (let chunkStart = 0; chunkStart < items.length; chunkStart += chunkSize) {
      ...
      const next = (async () => {
        ...
        const runOne = async (i: number): Promise<void> => {
          // Already settled on a prior attempt â€” reuse the persisted outcome.
  ...
  // Convergence loop (`converge`) â€” AGENT-COOPERATION-10X Â§Pillar 1.
  ...
  // whole cohort (Â§Pillar 3), and stops on goal / stall / budget / ceiling with
  // an honest terminal verdict. No graph cycle â€” the body subflow is re-invoked.
  // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  async #runConverge(
    ...
    // Durable resume â€” pick up persisted iteration state after a crash recovery.
    ...
    // Â§Pillar 3: one worktree the whole cohort shares for the loop lifetime.
    ...
    // ASSESS + REFLECT (COGNITIVE-LOOPING-RFC). `assess` off + `maxPivots` 0 â†’
    ...
    // acceptance-pass fraction) â†’ enables the plateau (s3) + regression (s4)
    ...
    try {
      // ASSESS on entry (RFC Â§7.4): on a durable resume, if the prior output
      // already satisfies the done-check, settle WITHOUT acting â€” the cheapest
      ...
      while (verdictKind !== 'goal_met' && iteration < maxIterations) {
        // Budget breaker â€” wall clock (always) + real recorded cost / tokens
        ...
        const iterInput: Record<string, unknown> = {
          ...
          converge: {
            ...
            // changes tack this pass instead of repeating (RFC Â§8).
            reflection: pendingReflection,
          },
        };
        pendingReflection = undefined; // one-shot â€” consumed by this iteration

        const rawOutput = await this.#runConvergeIteration(ctx, node, config, iterInput, iteration);
        // The body must not feed our control envelope back into shared state â€” a
        // body that echoes its inputs would otherwise create a self-referential
        // cycle (state â†’ converge â†’ state) that breaks run-state serialization.
        ...
        // ASSESS + multi-signal stagnation detection (RFC Â§3.2/Â§6). Measure the
        ...
        this.host.deps.bus.publish(REALTIME_ROOMS.run(ctx.runId), REALTIME_EVENTS.CONVERGE_ITERATION, {
          ...
          nodeId: node.id,
          ...record,
          // ASSESS â€” distance-to-goal, so the canvas can show "progress N%".
        ...
        // `stalled` â€” the "pivot, don't quit" rule (RFC Â§8).
      ...
      // workspace memory â€” but only when the goal was actually met, and only the
      ...
      const result: Record<string, unknown> = {
        ...
        output: lastOutput,
        state: accumulated,
        // ASSESS/REFLECT observability â€” the progress trajectory and any
        // self-critiques the Pursuit generated on the way (RFC Â§3.2/Â§8).
  ...
  async #evaluateConvergeContinuation(
    ...
    if (cont.type === 'objective') {
      ...
      // world-probe path the run-settle verdict uses â€” no self-report trusted.
      ...
      if (!spec || !spec.acceptance?.length) {
        ...
        // No scoped Objective to verify against â€” stop honestly rather than spin.
      ...
      return {
        ...
        // Progress = fraction of acceptance checks passing â†’ a real distance-to-goal (0..10).
    ...
    if (!evaluator) {
      ...
      // Without a judge we cannot assess convergence â€” stop to avoid an unbounded loop.
  ...
  async #acquireConvergeWorktree(
    ...
    try {
      const handle = await this.host.deps.worktrees.acquire({
        ...
        commitMessage: `Agentis cooperative loop â€” ${node.title ?? node.id}`,
      });
      if (!handle.path) {
  ...
  #convergeBudgetExceeded(
    ...
    if (!spend) return false; // No real signal â†’ don't fabricate enforcement.
  ...
  async #promoteConvergedKnowledge(
    ...
    try {
      ...
      // Drop disputed/revised claims â€” keep only the surviving truth.
...
/** Honest terminal verdicts â€” never a fake green. */
...
interface ConvergeIterationRecord {
  ...
  continue: boolean;
  /** Continuation verdict: open | converged | pass | fail | signalled_done | â€¦ */
  verdict: string;
  score?: number;
  critique?: string;
  /** Consecutive no-change iterations leading up to (and including) this one. */
  stallStreak: number;
  /** ASSESS â€” distance-to-goal progress 0..1 for this iteration (graded checks only). */
  progress?: number;
  /** Which stagnation signals fired this iteration (RFC Â§6). */
  stallReasons?: string[];
}

/** Persisted controller state â€” enough to resume a loop mid-flight after a crash. */
interface ConvergeRunState {
  ...
  lastOutput?: Record<string, unknown>;
  stallStreak?: number;
  /** ASSESS â€” progress 0..1 per iteration (the PRM-style trajectory). */
  deltaTrajectory?: number[];
  /** REFLECT â€” verbal self-critiques generated on stalls, fed forward. */
  reflections?: string[];
  /** REFLECT â€” pivots spent, so a resume respects the pivot budget. */
  pivotsUsed?: string[];
 ...
 * a `state â†’ converge â†’ state` cycle).
 */
