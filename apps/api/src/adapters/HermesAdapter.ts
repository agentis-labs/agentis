 ...
 *
 * Hermes is not routed through the generic HttpAdapter because operators expect
 ...
 * protocol used by vLLM and hosted Nous/Hermes endpoints. Local Ollama,
 ...
 * requiring AGENTIS_SKILL_HTTP_ALLOW_PRIVATE.
 */
...
export class HermesAdapter implements AgentAdapter {
  ...
  async *chat(messages: ChatMessage[], tools: ToolDefinition[]): AsyncIterable<ChatDelta> {
    ...
    yield* this.#streamChatDeltas(response);
  }

  async *#streamChatDeltas(response: Response): AsyncIterable<ChatDelta> {
    ...
    if (contentType.includes('application/json')) {
      ...
      for (const tc of extractToolCallsFull(rawCalls)) {
        yield { type: 'tool_call', id: tc.id, name: tc.name, args: tc.arguments };
      }
      const fr = choice?.finish_reason;
      yield { type: 'done', finishReason: fr === 'tool_calls' ? 'tool_calls' : 'stop' };
      return;
    }
...
async function assertLocalModelUrl(raw: string): Promise<URL> {
  ...
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    throw new AgentisError('SKILL_NETWORK_VIOLATION', 'Local model endpoints must use http(s)');
  }
  ...
  if (literal) {
    ...
    throw new AgentisError('SKILL_NETWORK_VIOLATION', `Local model endpoint must be localhost or a private network address; got ${host}`);
  }
  ...
  try {
    ...
    throw new AgentisError('SKILL_NETWORK_VIOLATION', `Could not resolve local model host ${host}: ${(err as Error).message}`);
  }
  if (addresses.length > 0 && addresses.every(isPrivateAddress)) return parsed;
  throw new AgentisError('SKILL_NETWORK_VIOLATION', `Local model endpoint ${host} must resolve only to localhost or private network addresses`);
}
