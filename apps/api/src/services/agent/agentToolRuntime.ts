...
import type { WorkflowStoreService } from '../workflow/workflowStore.js';
  ...
  async #run(workspaceId: string, tool: AgentTool, args: Record<string, unknown>, context: AgentToolContext): Promise<unknown> {
    switch (tool) {
      ...
      case 'read_url': {
        ...
        const res = await safeFetch(url, { timeoutMs: 20_000 }, { allowPrivate: false });
        const html = await res.text();
        return { url: res.url || url, status: res.status, text: stripHtml(html).slice(0, 20_000) };
      }
      case 'call_workflow': {
...
function stripHtml(html: string): string {
  return collapseWhitespace(decodeHtmlSpaceEntities(stripHtmlTagsAndUnsafeBlocks(html))).trim();
}

function stripHtmlTagsAndUnsafeBlocks(value: string): string {
  let out = '';
  ...
  for (let i = 0; i < value.length; i += 1) {
    ...
    if (startsTag(lower, i, 'script')) {
      const close = lower.indexOf('</script', i + 7);
      if (close < 0) break;
      const closeEnd = value.indexOf('>', close + 8);
      i = closeEnd < 0 ? value.length : closeEnd;
      out += ' ';
      continue;
    }
    if (startsTag(lower, i, 'style')) {
      const close = lower.indexOf('</style', i + 6);
      if (close < 0) break;
      const closeEnd = value.indexOf('>', close + 7);
      i = closeEnd < 0 ? value.length : closeEnd;
      out += ' ';
      continue;
    }
...
function startsTag(lowercaseHtml: string, index: number, tag: 'script' | 'style'): boolean {
  if (!lowercaseHtml.startsWith(`<${tag}`, index)) return false;
  const next = lowercaseHtml[index + tag.length + 1];
  return next === undefined || next === '>' || next === '/' || next === ' ' || next === '\t' || next === '\n' || next === '\r';
}

function decodeHtmlSpaceEntities(value: string): string {
  let out = '';
  for (let i = 0; i < value.length; i += 1) {
    if (value[i] !== '&') {
      out += value[i];
    ...
    const key = value.slice(i + 1, semi).toLowerCase();
    out += key === 'nbsp' ? ' ' : value.slice(i, semi + 1);
    i = semi;
  }
  return out;
}
