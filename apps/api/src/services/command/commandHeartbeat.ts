/**
 * CommandHeartbeat â€” proactivity (COMMAND-MODEL Layer C).
 *
 ...
 *
 *   â€¢ SURFACE (default, safe, zero spend) â€” log the attention signal + emit an
 ...
 *   â€¢ ACT (opt-in) â€” when autonomy is enabled AND a turn-runner is wired, drive a
...
export interface CommandHeartbeatDeps {
   ...
   * ChatSessionExecutor in bootstrap). Absent â‡’ surface-only. Should self-bound.
   */
...
export class CommandHeartbeat {
  /** Agents with an autonomous turn currently in flight â€” prevents double-running. */
  ...
  async #reviewOne(workspaceId: string, agentId: string): Promise<boolean> {
    ...
    if (this.deps.autonomyEnabled?.(workspaceId) && this.deps.runManagerTurn) {
      ...
      const message =
        ...
        + 'Call agentis.command.review first, then act through your tools â€” resolve approvals, replay/fix failed runs, dispatch specialists. '
...
// â”€â”€ heartbeat signature (workspace_kv) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ per-workspace autonomy opt-in (workspace_kv) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Autonomous action requires TWO switches ON: the global env master
// (AGENTIS_COMMAND_AUTONOMY) AND this per-workspace opt-in â€” so enabling autonomy
