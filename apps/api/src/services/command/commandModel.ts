/**
 * CommandModel â€” the fractal management mind (COMMAND-MODEL Layer B).
 *
 ...
 *
 *   1. Inventory  â€” what this agent owns (scoped via CommandScope).
 *   2. Progress   â€” run outcomes in a window + the DELTA since the agent last
 *                   reviewed (a persisted watermark), + what needs it now.
 *   3. Minds      â€” the App minds (appLearning.recentLearnings) that never reached
 ...
 * workspace) and a domain manager (its sector) â€” a fractal.
 */
...
const WINDOW_MS = 7 * DAY_MS;          // 7d â€” the primary progress window (counts/attention/delta).
const OUTCOME_WINDOW_MS = 30 * DAY_MS; // 30d â€” the widest window, for the outcome momentum trend.
...
export interface CommandProgress {
  ...
  attention: string[];
  /**
   * Semantic outcomes â€” declared workflow output labels (leads_qualified,
   * meetings_booked, â€¦) counted per COMPLETED run in-window. This is what the
  ...
  /** Outcome counts across 24h / 7d / 30d â€” the momentum of achieved work. */
...
export class CommandModelService {
  ...
  /** Build the full model for an agent. Never throws â€” degrades to inventory-only. */
  ...
  #progress(workspaceId: string, agentId: string, scope: CommandScope, workflowIds: string[] | null): CommandProgress {
    ...
    for (const r of runs7) {
      ...
      attention.push(`run ${r.id.slice(0, 8)} of "${titles.get(r.workflowId ?? '') ?? r.workflowId ?? 'workflow'}" FAILED${err ? ` â€” ${err}` : ''}`);
    }
   ...
   * accelerating" â€” not just how many runs finished. `runs` is the 30d set.
   */
  #outcomeTrend(
    workspaceId: string,
  ...
  #appLearnings(workspaceId: string, scope: CommandScope): Array<{ app: string; lessons: string[] }> {
    ...
    for (const appId of appIds) {
      ...
      } catch { /* App mind is best-effort â€” never break the briefing. */ }
    }
    return out;
  }
}

// â”€â”€ watermark (workspace_kv) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export function formatBriefing(m: CommandModel): string {
  const roleLabel = m.scope.kind === 'workspace' ? 'ORCHESTRATOR (whole workspace)'
    ...
    : 'OWNER of your work';
  const lines: string[] = [
    `COMMAND MODEL â€” you are the ${roleLabel}.`,
    `You manage ${m.inventory.apps} app(s), ${m.inventory.workflows} workflow(s), and ${m.inventory.specialists} specialist(s) across ${m.inventory.scopeLabel}. This is YOURS to run â€” manage it, don't just answer questions about it.`,
    `Progress (${m.progress.windowLabel}): ${m.progress.runsCompleted} completed Â· ${m.progress.runsFailed} failed Â· ${m.progress.runsInMotion} in motion.`,
  ];
  ...
  if (wideLabels.length > 0) {
    ...
    lines.push('Outcomes achieved (24h Â· 7d Â· 30d) â€” is the work accelerating?');
    for (const { label } of wideLabels.slice(0, OUTCOME_TREND_LABELS)) {
      lines.push(`- ${label}: ${at('24h', label)} Â· ${at('7d', label)} Â· ${at('30d', label)}`);
    }
  }
  ...
  if (m.appLearnings.length > 0) {
    lines.push('What your apps have learned (App minds â€” use these, they are yours):');
  ...
  lines.push(
    ...
    + 'Reach for anything you own as a live tool â€” apps, deep nodes, specialists, integrations/MCP â€” via agentis.capability.invoke. Progress the work; do not wait to be asked twice.',
  );
  return lines.join('\n');
}
