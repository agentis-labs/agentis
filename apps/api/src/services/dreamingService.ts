...
import {
  ...
  type PeerCardFact,
  type PeerRepresentationService,
  type PeerType,
  type VolatilityClass,
} from './peerRepresentationService.js';
...
type PeerConclusionRow = typeof schema.peerRepresentationConclusions.$inferSelect;
...
export class DreamingService {
  ...
  async runDue(workspaceId: string, options: { force?: boolean; phase?: 'deduction' | 'induction' | 'both'; limit?: number } = {}): Promise<DreamPassResult> {
    ...
    const peers = this.db.select().from(schema.peerRepresentations)
      .where(eq(schema.peerRepresentations.workspaceId, workspaceId))
      .orderBy(desc(schema.peerRepresentations.updatedAt))
      .limit(limit)
      .all()
  ...
  #activeConclusions(workspaceId: string, peerId: string, observerPeerId: string | null) {
    return this.db.select().from(schema.peerRepresentationConclusions)
      .where(and(
        ...
        eq(schema.peerRepresentationConclusions.status, 'active'),
      ))
      .orderBy(desc(schema.peerRepresentationConclusions.createdAt))
      .limit(250)
      .all()
  ...
  #supersedeStaleVolatileFacts(workspaceId: string, rows: PeerConclusionRow[]): number {
    ...
    for (const group of groups.values()) {
      ...
      for (const row of older) {
        this.db.update(schema.peerRepresentationConclusions)
          .set({ status: 'archived', supersededById: keeper!.id, updatedAt: now })
          .where(and(eq(schema.peerRepresentationConclusions.workspaceId, workspaceId), eq(schema.peerRepresentationConclusions.id, row.id)))
          .run();
        changed += 1;
      }
    }
...
function conclusionToFact(row: typeof schema.peerRepresentationConclusions.$inferSelect, now: string): PeerCardFact | null {
