...
export class BrainHealthService {
  ...
  snapshot(workspaceId: string, appId?: string | null): BrainHealthSnapshot {
    ...
    const events = this.db.select().from(schema.brainQualityEvents)
      ...
      .all()
      .filter((event) => !appId || !event.appId || event.appId === appId);
    ...
    const activeRows = this.db.select().from(schema.memoryEpisodes)
      .where(and(
        ...
        ...(appId ? [or(eq(schema.memoryEpisodes.appId, appId), isNull(schema.memoryEpisodes.appId))!] : []),
      ))
      .all()
