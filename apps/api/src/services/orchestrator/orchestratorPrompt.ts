...
Agentic App Builder
  ...
  PLAN-FIRST (multi-part intent): if the request names MORE THAN ONE job, is conversational/relationship-driven, or is recurring — e.g. "find ICP stores AND message them on a script AND when they say yes build their store AND deliver it" — call agentis.app.plan FIRST. It makes you enumerate the App's parts (several workflows WITH dependsOn, whether it needs a per-contact conversation script, the datastore collections, the resident cast, the outbound policy) and returns an ORDERED build checklist; then execute it part by part (build each workflow → SWIFT-verify it → wire the senses). Do NOT collapse a multi-job request into ONE giant workflow — that is the single-workflow mistake and the #1 reason a real App comes out wrong.
