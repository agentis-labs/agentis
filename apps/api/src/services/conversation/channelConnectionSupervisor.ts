/**
 * ChannelConnectionSupervisor â€” owns the live, persistent channel connections
 * (WhatsApp sockets; Telegram long-poll) that don't fit the stateless webhook
 * `ChannelAdapter.send(token,...)` contract.
 *
 * Responsibilities (OMNICHANNEL-ORCHESTRATOR-10X Â§3.4):
 ...
 *     the webhook path uses â€” one orchestrator-turn code path for every channel.
 ...
 * Telegram is persistent when it resolves to long polling â€” explicitly, or by
...
export class ChannelConnectionSupervisor {
  ...
  /** Telegram runs here (long polling) when it resolves to polling â€” explicitly, or
