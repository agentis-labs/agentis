...
export interface ResumeIngestionArgs {
  ...
  scopeId: string;
  jobId: string;
  /** Spec (re-loaded from the workspace manifest by the caller). */
  spec: DatasetSpec;
...
export class DatasetIngestion {
  ...
  /** Public read — used by /v1/brain ingestion jobs. */
    ...
    const rows = this.db
      ...
      .where(
        and(
          ...
          eq(schema.datasetImports.scopeId, args.scopeId),
  ...
  async #run(jobId: string, args: StartIngestionArgs, format: IngestionFormat): Promise<void> {
    ...
    for (let ci = 0; ci < chunks.length; ci++) {
      ...
      try {
        const storedId = this.#routeChunk(args.workspaceId, args.scopeId, args.spec, chunk, jobId);
    ...
    this.logger.info('dataset.ingestion.complete', {
      jobId, workspaceId: args.workspaceId, scopeId: args.scopeId,
  ...
  #buildImpactPreview(spec: DatasetSpec, stored: number): DatasetImpactPreview {
    ...
    if (spec.wedgeRole === 'primary_specialization' && stored > 0) {
      notes.push('This is a primary specialization dataset — the workspace should now feel domain-shaped.');
    }
    return {
...
function extractPdfText(content: string): string {
  ...
  while ((btMatch = btEtRe.exec(content)) !== null) {
    ...
    for (const literal of pdfLiteralOperands(block)) {
      parts.push(decodePdfLiteral(literal));
    }

    // Array string: [(items...)] TJ
    for (const arr of pdfArrayOperands(block)) {
      for (const literal of pdfLiteralStrings(arr)) {
        parts.push(decodePdfLiteral(literal));
      }
    }
...
function pdfLiteralOperands(block: string): string[] {
  const out: string[] = [];
  for (let i = 0; i < block.length; i += 1) {
    if (block[i] !== '(') continue;
    const literal = readPdfLiteral(block, i);
    if (!literal) continue;
    const op = skipWhitespace(block, literal.end + 1);
    if (block[op] === 'T' && (block[op + 1] === 'j' || block[op + 1] === 'J')) {
...
function pdfArrayOperands(block: string): string[] {
  const out: string[] = [];
  for (let i = 0; i < block.length; i += 1) {
    if (block[i] !== '[') continue;
    const end = findPdfArrayEnd(block, i);
    if (end < 0) continue;
    const op = skipWhitespace(block, end + 1);
    if (block[op] === 'T' && block[op + 1] === 'J') {
      out.push(block.slice(i + 1, end));
    }
    i = end;
  }
  return out;
...
function pdfLiteralStrings(source: string): string[] {
  const out: string[] = [];
  for (let i = 0; i < source.length; i += 1) {
    if (source[i] !== '(') continue;
    const literal = readPdfLiteral(source, i);
    if (!literal) continue;
    out.push(literal.value);
    i = literal.end;
  }
...
function readPdfLiteral(source: string, start: number): { value: string; end: number } | null {
  let escaped = false;
  let value = '';
...
function findPdfArrayEnd(source: string, start: number): number {
  for (let i = start + 1; i < source.length; i += 1) {
    const ch = source[i]!;
    if (ch === '(') {
      const literal = readPdfLiteral(source, i);
