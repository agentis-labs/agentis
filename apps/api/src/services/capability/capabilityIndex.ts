/**
 * CapabilityIndex â€” the compressed, searchable map of the whole workspace
 * (RADICAL-EFFICIENCY-CHAT-CAPABILITY-PLANE Â§1, "Efficient Global Awareness").
 *
 ...
 *
 *   â€¢ manifest(workspaceId)  â†’ a ~constant-size digest (counts + a few sample
 *     titles). Injected into every substantive chat turn so the agent KNOWS what
 *     exists without holding it â€” the "know-of-everything, load-almost-nothing"
 *     tier. Cheap: indexed reads, TTL-cached, no embeddings.
 *
 *   â€¢ search(workspaceId, intent) â†’ ranked capability atoms (URN + one-line
 ...
 * phases, agents, extensions) â€” no new table, no migration. Node and phase atoms
...
export interface CapabilityAtom {
  ...
  workflowId?: string;
  appId?: string;
  /** Relevance score, set on search results (0â€“1). */
  score?: number;
}
...
/** Node kinds that are pure canvas/boilerplate â€” never worth a search atom. */
const NOISE_NODE_KINDS = new Set(['sticky_note', 'trigger', 'return_output']);
/** Lexical prefilter width â€” how many candidates get semantically re-ranked. */
const SHORTLIST = 48;
...
export class CapabilityIndex {
  ...
  /** urn â†’ { hash, vector } embedding cache, keyed per workspace. */
  ...
  /** The compressed manifest â€” counts + sample titles. Cheap, no embeddings. */
  ...
  /**
   * Rank capability atoms against an intent. Hybrid: lexical prefilter â†’
   * semantic re-rank of the shortlist. Never throws â€” degrades to lexical order.
   */
  async search(
    workspaceId: string,
    intent: string,
    ...
    // Soft domain boost â€” rank the caller's own apps/workflows first WITHOUT
    ...
    try {
      ...
      for (const atom of shortlist) {
        ...
        const semantic = vec ? (cosineSimilarity(qVec, vec) + 1) / 2 : 0; // â†’ [0,1]
  ...
  #assembleAtoms(workspaceId: string): CapabilityAtom[] {
    ...
    for (const w of workflows) {
      ...
      // Node + phase atoms â€” the deep-targeting layer. Bounded per build.
      if (graph && scanned < MAX_WORKFLOWS_SCANNED) {
        ...
        for (const node of graph.nodes ?? []) {
          ...
          atoms.push({
            ...
            kind: 'node',
            title: `${w.title} â€º ${node.title || node.id}`,
    ...
    for (const g of agents) {
      ...
      atoms.push({
        ...
        kind: 'agent',
        title: g.name,
        purpose: oneLine([g.role, g.description ?? g.instructions ?? '', tags.slice(0, 4).join(' ')].filter(Boolean).join(' â€” ')),
      });
    }
...
export function formatManifest(m: CapabilityManifest): string {
  ...
  const lines: string[] = [
    'CAPABILITY MANIFEST',
    `Workspace "${m.workspaceName}" contains: ${parts.join(' Â· ')}.`,
    'You have COMPLETE awareness that these exist but hold none of them in context. To act:',
    '- agentis.capability.search(intent) â€” find the exact app/workflow/node/phase/agent by meaning (returns URNs).',
    '- agentis.capability.load(urns) â€” page in the full typed contract for the ones you pick.',
    '- agentis.capability.invoke(urn, input) â€” run it, down to a single node or phase (e.g. app:<id>/wf:<id>/node:<id>).',
  ];
  ...
  if (samples.length) lines.push(`For reference (not exhaustive) â€” ${samples.join(' | ')}.`);
  return lines.join('\n');
}
