 ...
 *
 * Responsibilities:
 *   1. Read/write the per-instance `agent_packages.app_graph` JSON column.
...
export class AppCanvasService {
  ...
  load(workspaceId: string, appId: string): AppCanvasRecord {
    ...
    const graph = (pkg?.appGraph as AppGraph | null | undefined) ?? emptyAppGraph();
  ...
  save(workspaceId: string, appId: string, graph: AppGraph): AppCanvasRecord {
    ...
    this.db
      .update(schema.agentPackages)
      .set({ appGraph: sanitized as unknown as object })
      .where(
        and(
  ...
  resetFromPackage(workspaceId: string, appId: string): AppCanvasRecord {
    ...
    const manifest = (pkg.manifest ?? {}) as Record<string, unknown>;
    const template = manifest.appGraphTemplate as AppGraph | undefined;
    const next = template ? sanitizeGraph(template) : emptyAppGraph();
    this.db
      .update(schema.agentPackages)
      .set({ appGraph: next as unknown as object })
      .where(
        and(
          eq(schema.agentPackages.id, appId),
          eq(schema.agentPackages.workspaceId, workspaceId),
  ...
  private collectReferences(
    workspaceId: string,
    appId: string,
    pkg: ReturnType<AppCanvasService['loadPackage']>,
      ...
      ? this.db
          ...
          .where(
            and(
              ...
              eq(schema.agents.packageId, pkg.id),
            ),
          )
          .all()
          .map((a) => ({
    ...
    const manifest = (pkg?.manifest ?? {}) as Record<string, unknown>;
    const datasetSpecs = (manifest.datasetSpecs as Array<{ key: string; label: string }>) ?? [];
    const integrationsSpec =
      (manifest.integrations as Array<{ service: string; name?: string }>) ?? [];
    const outputLabels =
      (manifest.outputLabels as Array<{ label: string; path: string }>) ?? [];
...
export function validateAppGraph(
  ...
  if (coreCount > 1) {
    errors.push({
      ...
      message: `Graph has ${coreCount} app_core nodes; exactly 1 is required.`,
      severity: 'error',
    });
  }
  if (graph.nodes.length > 0 && coreCount === 0) {
    warnings.push({
      ...
      message: 'Graph has no app_core node — add one to anchor the system.',
      severity: 'warning',
    });
  }
  if (graph.nodes.length > 0 && entryWorkflowCount === 0 && workflowModuleCount === 0) {
    warnings.push({
      ...
      message: 'Graph has no workflow nodes — apps usually run at least one workflow.',
      severity: 'warning',
    });
  }
    ...
    if (
      ...
      if (cfg.workflowId && !workflowIds.has(cfg.workflowId)) {
        errors.push({
          ...
          message: `Node '${node.title}' binds to unknown workflowId '${cfg.workflowId}'.`,
          severity: 'error',
          nodeId: node.id,
        });
      }
      if (!cfg.workflowId) {
        warnings.push({
          ...
          message: `Node '${node.title}' has no workflowId bound yet.`,
          severity: 'warning',
          nodeId: node.id,
        });
      }
      ...
      for (const id of cfg.agentIds ?? []) {
        if (!agentIds.has(id)) {
          warnings.push({
            ...
            message: `Agent group '${node.title}' references unknown agentId '${id}'.`,
            severity: 'warning',
            nodeId: node.id,
          });
        }
    ...
    if (cfg && 'kind' in cfg && cfg.kind === 'knowledge_source') {
      if (cfg.datasetKey && !datasetKeys.has(cfg.datasetKey)) {
        errors.push({
          ...
          message: `Knowledge source '${node.title}' binds to unknown datasetKey '${cfg.datasetKey}'.`,
          severity: 'error',
          nodeId: node.id,
        });
      }
    ...
    if (cfg && 'kind' in cfg && cfg.kind === 'integration_surface') {
      if (cfg.service && integrationKeys.size > 0 && !integrationKeys.has(cfg.service)) {
        warnings.push({
          ...
          message: `Integration surface '${node.title}' uses service '${cfg.service}' not declared in package manifest.integrations.`,
          severity: 'warning',
          nodeId: node.id,
        });
      }
    ...
    if (cfg && 'kind' in cfg && cfg.kind === 'output_surface') {
      if (cfg.outputKey && outputKeys.size > 0 && !outputKeys.has(cfg.outputKey)) {
        warnings.push({
          ...
          message: `Output surface '${node.title}' references outputKey '${cfg.outputKey}' not declared in manifest.outputLabels.`,
          severity: 'warning',
          nodeId: node.id,
        });
      }
  ...
  if (counts.knowledge_source === 0 && graph.nodes.length > 0) {
    warnings.push({
      ...
      message: 'No knowledge_source node — most apps benefit from explicit data inputs.',
      severity: 'warning',
    });
  }
  if (counts.output_surface === 0 && graph.nodes.length > 0) {
    warnings.push({
      ...
      message: 'No output_surface node — operators won\'t see what this app produces.',
      severity: 'warning',
    });
  }
  if (workflowModuleCount > 7) {
    warnings.push({
      ...
      message: `Graph exposes ${workflowModuleCount} top-level workflow_module nodes. The first-zoom rule (§6.3) recommends ≤7.`,
      severity: 'warning',
    });
  }
