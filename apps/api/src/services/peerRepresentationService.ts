...
export class PeerRepresentationService {
  queue?: BrainPromotionQueueWorker;
  ...
  getConclusions(workspaceId: string, peerId: string, limitOrOptions: number | PeerConclusionQueryOptions = 10): PeerConclusion[] {
    ...
    const rows = this.db.select().from(schema.peerRepresentationConclusions)
      .where(and(
        eq(schema.peerRepresentationConclusions.workspaceId, workspaceId),
        eq(schema.peerRepresentationConclusions.subjectPeerId, peerId),
      ))
      .orderBy(desc(schema.peerRepresentationConclusions.confidence), desc(schema.peerRepresentationConclusions.createdAt))
  ...
  async upsertPeerCardFacts(args: {
    ...
    if (!isDirectional || args.promoteToGlobal) {
      ...
      this.db.update(schema.peerRepresentations)
        .set({
          peerCard: merged,
          summary,
        ...
        .where(eq(schema.peerRepresentations.id, global.id))
        .run();
      count = merged.length;
    }
  ...
  async upsertFromSession(args: {
    ...
    for (const fact of factCandidates.slice(0, 12)) {
      ...
      this.db.insert(schema.peerRepresentationConclusions).values({
        id: randomUUID(),
        workspaceId: args.workspaceId,
    ...
    this.logger.info('peer_representation.updated', {
      workspaceId: args.workspaceId,
      peerId: args.peerId,
      sessionId: args.sessionId,
      conclusions: inserted,
  ...
  #dreamDue(workspaceId: string, peerId: string, inserted: number, observerPeerId?: string | null): boolean {
    ...
    const row = observerPeerId && observerPeerId !== peerId
      ...
      : this.db.select().from(schema.peerRepresentations)
        .where(and(eq(schema.peerRepresentations.workspaceId, workspaceId), eq(schema.peerRepresentations.peerId, peerId)))
        .get();
  ...
  #getGlobalRepresentation(workspaceId: string, peerType: PeerType, peerId: string) {
    return this.db.select().from(schema.peerRepresentations)
      .where(and(
        eq(schema.peerRepresentations.workspaceId, workspaceId),
        eq(schema.peerRepresentations.peerType, peerType),
        eq(schema.peerRepresentations.peerId, peerId),
      ))
      .get();
  }
  ...
  async #ensureGlobalRepresentation(workspaceId: string, peerType: PeerType, peerId: string, summary: string | null) {
    ...
    this.db.insert(schema.peerRepresentations).values(row).run();
    return row;
  }
  ...
  #loadSessionMessages(workspaceId: string, sessionId: string): Array<{ role: string; text: string }> {
    if (sessionId.startsWith('app:')) {
      const appId = sessionId.slice(4);
      return this.db.select().from(schema.appThreadMessages)
  ...
  async #embed(workspaceId: string, text: string): Promise<number[] | null> {
    ...
    try {
      ...
      this.logger.warn('peer_representation.embed_failed', { workspaceId, message: (err as Error).message });
      return null;
    }
  }
}

function rowToConclusion(row: typeof schema.peerRepresentationConclusions.$inferSelect): PeerConclusion {
  return {
    id: row.id,
