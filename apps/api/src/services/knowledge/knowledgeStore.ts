/**
 * KnowledgeStore — Workspace Knowledge Plane 5 backend.
 *
 *
 ...
 *   2. Read by scope + filters (source, tag) for the API surface.
 *   3. Retrieval:
...
const MAX_LIMIT = 50;
const CANDIDATE_SCAN_LIMIT = 2_000;
...
export class KnowledgeStore {
  ...
  /** List chunks for a workspace (UI listings, NOT retrieval). */
  list(args: {
    workspaceId: string;
    scopeId: string;
    source?: KnowledgeChunk['source'];
    ...
    const where = args.source
      ? and(
          ...
          eq(schema.knowledgeChunks.scopeId, args.scopeId),
  ...
  search(args: KnowledgeSearchArgs): KnowledgeHit[] {
    ...
    const sourceCondition = args.sources && args.sources.length > 0
      ? inArray(schema.knowledgeChunks.source, args.sources)
      : undefined;
    const tokenCondition = args.mode === 'vector'
      ? undefined
      : or(...queryTokens.slice(0, 8).map((token) => or(
        like(schema.knowledgeChunks.title, `%${token}%`),
        like(schema.knowledgeChunks.content, `%${token}%`),
      )!));

    // Bound the candidate set before scoring. Retrieval still ranks in JS for
    // explainability, but SQLite now handles workspace/scope/source/text prefilter
    // and recency ordering so large imports do not force full-table scans.
  ...
  /** Count helpers (used by the intelligence response summary). */
  countByScope(workspaceId: string, scopeId: string): { total: number; bySource: Record<string, number> } {
    const rows = this.db
      ...
      .where(
        and(
          ...
          eq(schema.knowledgeChunks.scopeId, scopeId),
        ),
      )
