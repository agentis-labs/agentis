 ...
 *   2. Read by workspace/scope/workflow/run/type for the API surface.
 *   3. Lexical and (when wired) semantic retrieval for the memory runtime.
 *
 * Distinct from the wedge's `MemoryStore` (which holds `workspace_memory` rows
...
export interface EpisodeSearchArgs {
  workspaceId: string;
  /** Restrict to one intelligence scope; falsy means workspace-wide. */
  scopeId?: string;
  workflowId?: string;
...
export class EpisodicMemoryStore {
  ...
  /** List episodes for a workspace/workspace. UI listings, not retrieval ranking. */
  list(args: {
    workspaceId: string;
    scopeId?: string;
    types?: RuntimeEpisodeType[];
  ...
  searchEpisodes(args: EpisodeSearchArgs): RuntimeEpisode[] {
    ...
    // Load candidates for the workspace (and optionally one scope).
    const conds = [eq(schema.memoryEpisodes.workspaceId, args.workspaceId)];
    if (args.scopeId) conds.push(eq(schema.memoryEpisodes.scopeId, args.scopeId));
    if (args.workflowId) conds.push(eq(schema.memoryEpisodes.workflowId, args.workflowId));
    if (!args.includeArchived) conds.push(isNull(schema.memoryEpisodes.archivedAt));
  ...
  findSimilar(workspaceId: string, candidate: { title: string; summary: string; scopeId?: string | null; type?: string }, threshold = 0.7): RuntimeEpisode[] {
    const args: EpisodeSearchArgs = {
      workspaceId,
      query: `${candidate.title} ${candidate.summary}`,
      limit: 5,
    };
    if (candidate.scopeId) args.scopeId = candidate.scopeId;
  ...
  countByScope(workspaceId: string, scopeId: string): { total: number; byType: Record<string, number> } {
