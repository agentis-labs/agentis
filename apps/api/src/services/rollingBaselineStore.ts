 ...
 *
 * Spec: docs/memory/MEMORY-ARCHITECTURE.md §8.
 *
...
export class RollingBaselineStore {
  ...
  /** All latest baselines for an app (one per workflow×window). */
  latestForApp(workspaceId: string, appId: string): RollingBaselineSnapshot[] {
    const rows = this.db.select().from(schema.rollingBaselineSnapshots)
      .where(
        and(
          eq(schema.rollingBaselineSnapshots.workspaceId, workspaceId),
          eq(schema.rollingBaselineSnapshots.appId, appId),
        ),
      )
      .orderBy(desc(schema.rollingBaselineSnapshots.capturedAt))
      .all();
  ...
  /** Delete all snapshots for an app (uninstall). */
  deleteForApp(workspaceId: string, appId: string): number {
    const result = this.db.delete(schema.rollingBaselineSnapshots)
      .where(
        and(
          eq(schema.rollingBaselineSnapshots.workspaceId, workspaceId),
          eq(schema.rollingBaselineSnapshots.appId, appId),
