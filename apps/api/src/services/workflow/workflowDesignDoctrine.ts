...
export const WORKFLOW_DESIGN_DOCTRINE = [
  ...
  '  D9. Is this ONE job, or SEVERAL? D1–D8 stop at the graph boundary; this one asks',
  '      whether the boundary is in the right place. If the request names more than one',
  '      job ("find leads AND message them AND build their store"), or the parts have',
  '      DIFFERENT triggers (one recurring, one reacting to a reply), different cadences,',
  '      or would be useful separately — do NOT collapse them into one graph. Use',
  '      agentis.app.plan to split them, then agentis.workflow.chain to wire the order.',
  '      A chain link is richer than an edge: it carries a condition (`when`), an error',
  '      branch (chainOn:"failure"), a delay, and the upstream\'s real output — and',
  '      dependsOn is a true JOIN (a workflow depending on [A,C] runs once, when both',
  '      are done). One giant graph re-runs everything to change anything.',
  '  D10. HOW DOES IT START? A workflow with a manual trigger runs only when a person',
  '      clicks it. Decide deliberately: recurring → cron; reacting to an inbound call →',
  '      webhook; watching a source → persistent_listener; running after another workflow',
  '      → dependsOn (NOT a trigger). Declare it with build_workflow `trigger` rather than',
  '      hoping the description implies it — unstated means MANUAL, which is how',
  '      unattended work silently becomes work nobody ever starts.',
