...
import { mcpCatalogForConnector } from '../mcp/mcpServerCatalog.js';

const RAL_AGENT_NODE_KINDS = new Set(['agent_task', 'agent_session', 'agent_swarm', 'dynamic_swarm']);
...
export function analyzeWorkflowReadiness(
  ...
  // RAL supply view: what each workspace agent's runtime can provide by config
  // (and could provide if enabled) — derived from the agent row, no live adapter
  // needed. Lets us flag a node whose RAL `requires` no runtime can satisfy
  ...
  for (const node of graph.nodes) {
    ...
    if (cfg.kind === 'trigger') {
    ...
    } else if (RAL_AGENT_NODE_KINDS.has(String(cfg.kind ?? ''))) {
      // `fileSystem` is covered by the platform tool loop itself; every other RAL
      ...
      const satisfiable = ralAgents.some((agent) => required.every((key) => agent.configured[key] === true));
      if (satisfiable) continue;
      const enablable = ralAgents.find((agent) => required.every((key) => agent.potential[key] === true));
      requirements.push({
        nodeId: node.id,
        nodeTitle: title,
        kind: 'config',
