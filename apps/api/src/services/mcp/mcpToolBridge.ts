/**
 * McpToolBridge â€” make external MCP servers callable as agent tools
 ...
 * were only reachable over REST (`/v1/mcp-servers/:id/call`) â€” never offered to
 ...
 *      `AGENTIS_COMPUTER_USE_MCP_URL`) â€” the operator opts in by pointing Agentis
...
export interface BridgedToolSpec {
  ...
  toolName: string;
  description: string;
  /** The RAL affordance this server grants, when tagged (e.g. `computerUse`). */
...
export class McpToolBridge {
  ...
  async #serverTools(workspaceId: string, server: McpServerConfig, usedSlugs: Map<string, number>): Promise<BridgedToolSpec[]> {
    ...
    // Least privilege: a non-empty allowlist makes ONLY those tools visible â€”
    // to agents, to `mcp` nodes, and to REST â€” the rest of the server's
  ...
  #client(workspaceId: string, server: McpServerConfig): McpClientLike {
    ...
    // (JSON object â†’ headers; bare token â†’ Authorization: Bearer). The secret
...
/** True when a string is a recognized RAL affordance key. */
