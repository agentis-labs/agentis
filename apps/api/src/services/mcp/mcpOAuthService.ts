/**
 * McpOAuthService â€” spec-compliant OAuth 2.1 for external MCP servers
 ...
 *
 *   1. Probe the server â†’ 401 + `WWW-Authenticate` â†’ Protected Resource
 *      Metadata (PRM, RFC9728), or the `/.well-known/oauth-protected-resource`
 *      fallback â†’ the authorization server(s).
 *   2. Authorization Server Metadata (RFC8414,
 *      `/.well-known/oauth-authorization-server`) â†’ authorize / token /
 *      registration endpoints.
 *   3. Dynamic Client Registration (RFC7591) â†’ a `client_id`, no pre-made app.
 *   4. Authorization Code + PKCE (RFC7636) + Resource Indicator (RFC8707 = the
 *      server URL) â†’ redirect â†’ callback â†’ token exchange.
 *
 * The operator types NOTHING for an OAuth mount â€” this is the "Connect with X"
...
export interface McpAuthEndpoints {
  ...
  /** The resource identifier to bind the token to (RFC8707) â€” the server URL. */
  resource: string;
...
export class McpOAuthService {
   ...
   * advertise OAuth (no 401/PRM/well-known) â€” the caller then falls back to a
  ...
  async discover(serverUrl: string, allowPrivateNetwork = false): Promise<McpAuthEndpoints | null> {
    ...
    // 1. Probe â†’ the PRM location (WWW-Authenticate `resource_metadata`), else
    ...
    // 2. PRM (RFC9728) â†’ authorization_servers[].
  ...
  async exchangeCode(entry: StateEntry, code: string): Promise<McpOAuthTokens> {
    const body = new URLSearchParams({
      ...
      resource: entry.resource, // RFC8707 â€” same resource as the authorize step
    });
    // Confidential client (DCR returned a secret): authenticate via
    // `client_secret_post` â€” the shape Supabase et al. expect.
    ...
    if (!res.ok || typeof json.access_token !== 'string') {
      throw new AgentisError('INTEGRATION_OPERATION_FAILED', `MCP OAuth token exchange failed â€” ${tokenErrorMessage(json, raw, res.status)}`);
    }
    return {
   ...
   *  `client_secret` â€” we honor whatever they return. Throws if the server
  ...
  async #registerClient(registrationEndpoint: string | undefined, redirectUri: string, allowPrivate: boolean): Promise<{ clientId: string; clientSecret?: string }> {
    if (!registrationEndpoint) {
      throw new AgentisError('INTEGRATION_OPERATION_FAILED',
        'This server does not support Dynamic Client Registration â€” a manual OAuth client is required (not yet supported); use a token mount.');
    }
...
/** Parse the `resource_metadata="â€¦"` param from a WWW-Authenticate header. */
...
/** The well-known PRM location for a server URL (RFC9728 Â§3.1). */
...
function tokenErrorMessage(json: Record<string, unknown>, raw: string, status: number): string {
  ...
  return parts.length > 0 ? `${parts.join(' â€” ')} (HTTP ${status})` : `token endpoint returned ${status}`;
}
