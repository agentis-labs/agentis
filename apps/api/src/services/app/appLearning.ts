/**
 * AppLearningService â€” the conversational learning loop (LIVING-APPS-10X Phase M2 Â· G10).
 *
 * Closes the loop so a resident App agent gets MEASURABLY better over time:
 *
 *   1. OUTCOME  â€” a relationship reaches a terminal state (won | lost | abandoned),
 ...
 *   2. GRADED LESSON â€” on outcome, a distilled, durable lesson (what worked / what
 *      didn't for this App + role) is deposited through the EXISTING brain-formation
 *      path (`SharedIntelligence.addAtom` â†’ a `distilled_lesson` episode), scoped to
 *      the App's owner agent so it lands in that agent's memory plane â€” never a raw
 *      transcript, and it respects the formation/scrubbing the brain already does.
 *   3. GRADUATION â€” those lessons live in the SAME episodic plane the cross-session
 *      `MemoryReflectionService` already reflects over. When lessons of the same
 *      shape recur (its â‰¥2-distinct-sources + â‰¥3-for-skill thresholds), reflection
 *      derives a generalized rule and fires the EXISTING `SkillProposer` hook â€”
 ...
 *   4. VISIBILITY â€” `recentLearnings` returns the recent graded lessons + graduated
 ...
 * recurrence-reinforcement â€” it never fabricates a rule or an ability.
 */
 ...
 * reflection engine's â‰¥3-distinct-sources skill-proposal gate so the FIRST pass can
...
export interface AppLearningDeps {
  ...
  logger: Logger;
  /** Optional â€” triggers a scoped reflection pass so graduation is prompt. */
...
export class AppLearningService {
   ...
   * derive an outcome from a `stage â†’ 'won'/'lost'` transition with no new field.
   */
   ...
   * (graduation). Idempotent on the contact's terminal outcome â€” re-recording the
  ...
  async recordOutcome(input: RecordOutcomeInput): Promise<RecordOutcomeResult> {
    ...
    try {
      ...
      // Idempotency: same terminal outcome already recorded â†’ nothing to do.
      ...
      // GRADUATION â€” reflect over the (now-richer) scoped plane so recurring winning
      ...
      // lesson count crosses a multiple of GRADUATION_THRESHOLD. This matters â€” the
      // SkillProposer needs â‰¥3 DISTINCT sources, so a too-eager pass at 2 lessons would
      ...
      // first pass sees â‰¥3 lets graduation fire directly. The periodic brain-queue
   ...
   * terminal outcome â€” deposit a graded lesson to the App owner's memory plane,
   ...
   * relationship App's Brain (support, booking, sales, collectionsâ€¦) fill with
  ...
  async recordConversationOutcome(input: {
    ...
    try {
      ...
      result.lessonDeposited = true;
      // GRADUATION â€” same recurrence-gated nudge as recordOutcome.
   ...
   * (â†’ a graded lesson). Returns how many were swept. Non-throwing.
   */
  ...
  /**
   * "What this agent learned" â€” recent graded lessons + graduated abilities for an
   ...
   * NOT the transcript â€” it's a third-person, reusable rule the role should carry.
  ...
  #composeLesson(args: {
    ...
    if (note && note.length >= 12) {
      // The agent distilled it â€” keep it, framed for reuse.
      body = `When working a ${channel} relationship${goal}, a recently ${outcome} conversation taught: ${truncate(note, 320)}`;
    } else if (outcome === 'won') {
      body = `Relationships on ${channel} that reached "${stage}"${goal} were won â€” repeat the approach that advanced them through that stage; keep momentum and follow up as promised.`;
    } else if (outcome === 'lost') {
      body = `Relationships on ${channel} that stalled at "${stage}"${goal} were lost â€” treat that stage as a risk point; surface objections early and offer the next concrete step instead of letting it drift.`;
    } else {
      body = `Relationships on ${channel} left untouched at "${stage}"${goal} were abandoned â€” reach out before the thread goes cold; set a next-touch and follow up proactively rather than waiting.`;
    }
...
function truncate(value: string, max: number): string {
  return value.length <= max ? value : `${value.slice(0, Math.max(0, max - 1)).trim()}â€¦`;
}
