/**
 * AppOrchestrator â€” multi-workflow control BY RULE for an Agentic App
 * (APP-INTERFACE-10X-MASTERPLAN Â§2.3).
 *
 ...
 *
 *  1. **Chains** â€” when a run of workflow W (owned by app A) settles, every
 *     enabled sibling whose `dependsOn` includes W is queued â€” through the same
 ...
 *  2. **Schedules** â€” `binding.schedule.cron` fires on the SchedulerService sweep
 *     seam. Graph-authored triggers stay authoritative where present; this is the
 *     App-level "run this atâ€¦" layer for workflows without their own trigger.
 *  3. **Concurrency** â€” `binding.concurrency === 'exclusive'` skips an
 *     orchestrated start while a run of that workflow is still active (manual
 *     starts are never blocked â€” the operator outranks the rule).
 *  4. **Run-all** â€” start every enabled root (no dependsOn) in `order`; chains
 *     cascade from there.
 *
...
/** Cap on app-chain lineage depth â€” a dependsOn cycle terminates here. */
...
export class AppOrchestratorService {
  ...
  /** workflowId â†’ next scheduled fire (ms epoch). Rebuilt on boot + rearm(). */
  ...
  // â”€â”€ 1. dependsOn chains â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ 2. binding schedules (cron) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  /**
   * SchedulerService sweep hook â€” fire every due binding schedule. Missed windows
  ...
  async sweepSchedules(now = new Date()): Promise<number> {
    ...
    for (const [workflowId, dueAt] of [...this.#nextFire.entries()]) {
      ...
      const row = this.workflowBindingRow(workflowId);
      // Binding vanished or was disabled since arming â€” drop silently.
  ...
  // â”€â”€ 4. run-all â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
