/**
 * BrainPromotionQueueWorker — durable, restart-safe brain promotion.
 *
 * Spec: docs/BRAIN-ABILITIES-REPLAN.md §BL10 + Appendix B.
 *
 ...
 *
 *   - polls `brain_promotion_queue` every 5s; pending rows survive restarts
...
import type { AgentAbilityReviewer, AbilityReviewInput } from './agentAbilityReviewer.js';
import type { PeerRepresentationService } from './peerRepresentationService.js';
import type { DreamingService } from './dreamingService.js';
...
export class BrainPromotionQueueWorker {
  ...
  enqueue(args: EnqueueArgs): string {
    ...
    this.db.insert(schema.brainPromotionQueue).values({
      id,
  ...
  start(): void {
    ...
    this.#timer.unref?.();
    this.logger.info('brain_promotion_queue.started', { intervalMs: POLL_INTERVAL_MS });
  }
  ...
  async poll(): Promise<void> {
    ...
    try {
      const items = this.db
        .select()
        .from(schema.brainPromotionQueue)
        .where(and(
          or(
            eq(schema.brainPromotionQueue.status, 'pending'),
            and(
              eq(schema.brainPromotionQueue.status, 'processing'),
              lte(schema.brainPromotionQueue.updatedAt, new Date(Date.now() - PROCESSING_LEASE_MS).toISOString()),
            ),
          )!,
          lt(schema.brainPromotionQueue.attempts, MAX_ATTEMPTS),
        ))
        .orderBy(
          sql`CASE ${schema.brainPromotionQueue.priority} WHEN 'high' THEN 0 WHEN 'normal' THEN 1 ELSE 2 END`,
          asc(schema.brainPromotionQueue.createdAt),
        )
        .limit(CLAIM_BATCH)
        .all();
      ...
      for (const item of items) {
        ...
        const claimed = this.db.update(schema.brainPromotionQueue)
          .set({
            status: 'processing',
            attempts: sql`${schema.brainPromotionQueue.attempts} + 1`,
            lastAttemptAt: new Date().toISOString(),
            updatedAt: new Date().toISOString(),
          })
          .where(and(
            eq(schema.brainPromotionQueue.id, item.id),
            eq(schema.brainPromotionQueue.status, item.status),
            eq(schema.brainPromotionQueue.updatedAt, item.updatedAt),
          ))
          .run();
      ...
      this.logger.warn('brain_promotion_queue.poll_failed', { message: (err as Error).message });
    } finally {
      this.#polling = false;
    }
  }

  async #processItem(item: typeof schema.brainPromotionQueue.$inferSelect): Promise<void> {
    const now = new Date().toISOString();
    try {
      await this.#dispatch(item);
      this.db.update(schema.brainPromotionQueue)
        .set({ status: 'done', updatedAt: now, lastAttemptAt: now })
        .where(eq(schema.brainPromotionQueue.id, item.id))
        .run();
      ...
      this.logger.warn('brain_promotion_queue.item_failed', {
        id: item.id,
  ...
  async #dispatch(item: typeof schema.brainPromotionQueue.$inferSelect): Promise<void> {
    const payload = (item.payload ?? {}) as Record<string, unknown>;
    switch (item.itemType) {
      case 'atom_promotion': {
        const p = payload as unknown as AtomPromotionPayload;
        await this.collectiveBrain.promote({
      ...
      case 'ability_review': {
        if (!this.abilityReviewer) {
          this.logger.info('brain_promotion_queue.ability_review_skipped', { reason: 'no reviewer wired' });
          return;
        }
      ...
      case 'peer_update': {
        if (!this.peerRepresentations) {
          this.logger.info('brain_promotion_queue.peer_update_skipped', { reason: 'no peer service wired' });
          return;
        }
        const p = payload as {
          workspaceId?: string;
        ...
        await this.peerRepresentations.upsertFromSession({
          workspaceId: p.workspaceId,
      ...
      case 'contradiction_check': {
        ...
        const flagged = this.collectiveBrain.flagDispute({
          workspaceId: p.workspaceId,
          atomIdA: p.atomIdA,
          atomIdB: p.atomIdB,
        ...
        if (p.autoResolve && flagged.linkId) {
          await this.collectiveBrain.resolveDispute({
            workspaceId: p.workspaceId,
      ...
      case 'curator_pass': {
        const p = payload as { workspaceId?: string; atomIds?: string[]; clusterTag?: string; appId?: string | null };
        if (!p.workspaceId || !Array.isArray(p.atomIds) || p.atomIds.length === 0) return;
        await this.#runCuratorPass(p.workspaceId, p.atomIds, p.clusterTag ?? 'memory', p.appId ?? null);
        return;
      }
      case 'dream_pass': {
        if (!this.dreaming) {
          this.logger.info('brain_promotion_queue.dream_pass_skipped', { reason: 'no dreaming service wired' });
          return;
        }
        const p = payload as {
          workspaceId?: string;
      ...
      case 'reembed_workspace': {
        ...
        await this.collectiveBrain.reembedWorkspaceAtoms(p.workspaceId, p.requestId ?? null);
        return;
      }
      default:
        this.logger.info('brain_promotion_queue.item_skipped', { itemType: item.itemType });
        return;
    }
  }

  async #runCuratorPass(workspaceId: string, atomIds: string[], clusterTag: string, appId: string | null): Promise<void> {
    const rows = atomIds
    ...
    const created = await this.collectiveBrain.addAtom({
      workspaceId,
      appId: appId ?? rows.find((row) => row.appId)?.appId ?? null,
      content: `Distilled ${clusterTag}: ${content.slice(0, 900)}`,
      tags: [clusterTag, 'curator_distilled'],
      confidence,
      source: 'system_write',
      managed: true,
      metadata: { source: 'curator_distilled', compressedFrom: rows.map((row) => row.id) },
      compressionTier: 3,
  ...
  #recordFailure(workspaceId: string): void {
    ...
    if (state.failures >= CIRCUIT_FAIL_THRESHOLD) {
      ...
      this.logger.warn('brain_promotion_queue.circuit_open', {
        workspaceId,
