 ...
 * Plans are returned directly as clean chat Markdown. No secondary visual
 * payload or local plan file is required.
 */
export const PLAN_MODE_SYSTEM_ADDENDUM = [
  ...
  'Write the answer directly in the conversation as clean Markdown with concise headings and numbered steps.',
  'Never emit XML-like protocol tags, hidden JSON payloads, architecture_canvas blocks, or proposed_plan wrappers.',
  'Do not save the plan to a local runtime file or link to private file:// paths unless the operator explicitly asks for a file.',
  'For workflow, extension, app, or system design, explain the actual architecture in readable prose: triggers, data sources, specialist steps, integrations, guardrails, outputs, and verification.',
  'End with only the decisions genuinely needed from the operator. Do not ask them to open or review a separate local document.',
].join('\n');
