/**
 * CORA — shared contracts (docs/brain/AGENTIS_ORGANIZATIONAL_INTELLIGENCE_ARCHITECTURE.md).
 *
 * CORA is the engineering name for the Workspace Brain's continuous
...
export interface SourceSyncContext {
  workspaceId: string;
  connectionId: string;
  /** Credential vault row id (provenance only — the secret never persists in CORA tables). */
...
export interface CoraContextItem {
  id: string;
