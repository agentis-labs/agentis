/**
 * CORA Agent Continuity — grants + context composition (RFC §9.5, §12).
 *
 * This is NOT a new dispatch composer. SharedIntelligenceService keeps owning
 * buildDispatchContext; it calls setCoraComposer(this) at bootstrap (the same
...
export class CoraContextComposer {
  ...
  putGrant(args: {
    ...
    const existing = this.db.select().from(schema.coraAgentGrants)
      .where(and(eq(schema.coraAgentGrants.workspaceId, args.workspaceId), eq(schema.coraAgentGrants.agentId, args.agentId)))
      .get();
    const values = {
      mode: args.mode,
    ...
    if (existing) {
      this.db.update(schema.coraAgentGrants).set(values).where(eq(schema.coraAgentGrants.id, existing.id)).run();
      return this.getGrantRow(args.workspaceId, args.agentId)!;
    }
    this.db.insert(schema.coraAgentGrants).values({
      id: randomUUID(),
  ...
  getGrantRow(workspaceId: string, agentId: string) {
    return this.db.select().from(schema.coraAgentGrants)
      .where(and(eq(schema.coraAgentGrants.workspaceId, workspaceId), eq(schema.coraAgentGrants.agentId, agentId)))
      .get() ?? null;
  }

  deleteGrant(workspaceId: string, agentId: string) {
    this.db.delete(schema.coraAgentGrants)
      .where(and(eq(schema.coraAgentGrants.workspaceId, workspaceId), eq(schema.coraAgentGrants.agentId, agentId)))
      .run();
  }
  ...
  /**
   * Compose the CORA layer for one dispatch. Authorization applies BEFORE
  ...
  composeForDispatch(args: {
    ...
    const empty: CoraContextBundle = { items: [], influenceIds: [], grantMode: grant.mode, block: '' };
    ...
    // owner and dispatch proceeds without a CORA block. An approved request
    ...
    const claims = this.db.select().from(schema.coraClaims)
      .where(and(
        eq(schema.coraClaims.workspaceId, args.workspaceId),
        eq(schema.coraClaims.status, 'active'),
      ))
    ...
    const items: CoraContextItem[] = ranked.map(({ claim, score }) => ({
      id: claim.id,
    ...
    for (const item of items) {
      ...
      this.db.insert(schema.coraBehaviorInfluences).values({
        id: influenceId,
        workspaceId: args.workspaceId,
        agentId: args.agentId,
    ...
    if (audience !== 'private') {
      ...
      if (!verdict.ok) {
        this.deps.logger.warn('cora.disclosure.blocked', { workspaceId: args.workspaceId, agentId: args.agentId, violations: verdict.violations });
        return empty;
      }
    }
  ...
  private consumeApproval(workspaceId: string, agentId: string, runId: string | null): boolean {
    ...
    const approvals = this.db.select().from(schema.coraAccessRequests)
      .where(and(
        eq(schema.coraAccessRequests.workspaceId, workspaceId),
        eq(schema.coraAccessRequests.agentId, agentId),
        eq(schema.coraAccessRequests.status, 'approved'),
      ))
      .all()
    ...
    if (usable.decisionScope === 'once') {
      this.db.update(schema.coraAccessRequests)
        .set({ status: 'expired' })
        .where(eq(schema.coraAccessRequests.id, usable.id))
        .run();
    }
    return true;
  }
  ...
  private recordAccessRequest(args: { workspaceId: string; agentId: string; runId?: string | null; taskDescription: string; interactionAudience?: string }): void {
    const pending = this.db.select().from(schema.coraAccessRequests)
      .where(and(
        eq(schema.coraAccessRequests.workspaceId, args.workspaceId),
        eq(schema.coraAccessRequests.agentId, args.agentId),
        eq(schema.coraAccessRequests.status, 'pending'),
      ))
      .get();
    if (pending) return; // one open ask per agent — no request spam
    this.db.insert(schema.coraAccessRequests).values({
      id: randomUUID(),
      workspaceId: args.workspaceId,
      agentId: args.agentId,
      runId: args.runId ?? null,
  ...
  listAccessRequests(workspaceId: string, options: { status?: string } = {}) {
    const conditions = [eq(schema.coraAccessRequests.workspaceId, workspaceId)];
    if (options.status) conditions.push(eq(schema.coraAccessRequests.status, options.status));
    return this.db.select().from(schema.coraAccessRequests)
      .where(and(...conditions))
      .orderBy(desc(schema.coraAccessRequests.createdAt))
      .limit(100)
      .all();
  }
  ...
  decideAccessRequest(args: {
    ...
    this.db.update(schema.coraAccessRequests)
      ...
      .where(and(eq(schema.coraAccessRequests.workspaceId, args.workspaceId), eq(schema.coraAccessRequests.id, args.requestId)))
      .run();
    return this.db.select().from(schema.coraAccessRequests)
      .where(eq(schema.coraAccessRequests.id, args.requestId))
      .get() ?? null;
  }
  ...
  validateDisclosure(workspaceId: string, text: string): { ok: boolean; violations: string[] } {
    ...
    const restricted = this.db.select().from(schema.coraClaims)
  ...
  listInfluences(workspaceId: string, options: { agentId?: string; limit?: number } = {}) {
    const conditions = [eq(schema.coraBehaviorInfluences.workspaceId, workspaceId)];
    if (options.agentId) conditions.push(eq(schema.coraBehaviorInfluences.agentId, options.agentId));
    return this.db.select().from(schema.coraBehaviorInfluences)
      .where(and(...conditions))
      .orderBy(desc(schema.coraBehaviorInfluences.createdAt))
  ...
  revokeInfluence(workspaceId: string, influenceId: string) {
    const now = new Date().toISOString();
    this.db.update(schema.coraBehaviorInfluences)
      .set({ status: 'revoked', revokedAt: now })
      .where(and(eq(schema.coraBehaviorInfluences.workspaceId, workspaceId), eq(schema.coraBehaviorInfluences.id, influenceId)))
      .run();
  }
  ...
  revokeInfluencesForClaims(workspaceId: string, claimIds: string[]) {
    ...
    const influences = this.db.select().from(schema.coraBehaviorInfluences)
      .where(and(eq(schema.coraBehaviorInfluences.workspaceId, workspaceId), eq(schema.coraBehaviorInfluences.status, 'active')))
      .all()
    ...
    this.db.update(schema.coraBehaviorInfluences)
      .set({ status: 'revoked', revokedAt: now })
      .where(inArray(schema.coraBehaviorInfluences.id, influences.map((i) => i.id)))
      .run();
  }
