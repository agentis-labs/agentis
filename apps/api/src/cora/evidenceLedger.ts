/**
 * CORA Evidence Ledger — the durable boundary between untrusted source
...
export class EvidenceLedgerService {
  ...
  recordObject(args: {
    ...
    let row = this.db.select().from(schema.coraSourceObjects)
      .where(and(
        eq(schema.coraSourceObjects.workspaceId, args.workspaceId),
        eq(schema.coraSourceObjects.connectionId, args.connectionId),
        eq(schema.coraSourceObjects.externalId, sanitized.externalId),
      ))
      .get();
    if (!row) {
      const id = randomUUID();
      this.db.insert(schema.coraSourceObjects).values({
        id,
      ...
      row = this.db.select().from(schema.coraSourceObjects).where(eq(schema.coraSourceObjects.id, id)).get()!;
    }

    // 2. Hash the normalized content. Same hash ⇒ replay ⇒ no new version.
    const contentHash = this.hash(sanitized);
    const existing = this.db.select().from(schema.coraEvidenceVersions)
      .where(and(
        eq(schema.coraEvidenceVersions.sourceObjectId, row.id),
        eq(schema.coraEvidenceVersions.contentHash, contentHash),
      ))
      .get();
    if (existing) {
    ...
    this.db.insert(schema.coraEvidenceVersions).values({
      id: versionId,
      workspaceId: args.workspaceId,
    ...
    if (predecessorId) {
      this.db.update(schema.coraEvidenceVersions)
        ...
        .where(eq(schema.coraEvidenceVersions.id, predecessorId))
        .run();
    }
    this.db.update(schema.coraSourceObjects)
      .set({
      ...
      .where(eq(schema.coraSourceObjects.id, row.id))
      .run();
  ...
  recordDeletion(args: {
    ...
    const row = this.db.select().from(schema.coraSourceObjects)
      .where(and(
        eq(schema.coraSourceObjects.workspaceId, args.workspaceId),
        eq(schema.coraSourceObjects.connectionId, args.connectionId),
        eq(schema.coraSourceObjects.externalId, args.externalId),
      ))
      .get();
    ...
    this.db.update(schema.coraSourceObjects)
      ...
      .where(eq(schema.coraSourceObjects.id, row.id))
      .run();
    const versions = this.db.select({ id: schema.coraEvidenceVersions.id }).from(schema.coraEvidenceVersions)
      .where(eq(schema.coraEvidenceVersions.sourceObjectId, row.id))
      .all();
    const ids = versions.map((v) => v.id);
    if (row.currentVersionId) {
      this.db.update(schema.coraEvidenceVersions)
        .set({ validUntil: now })
        .where(eq(schema.coraEvidenceVersions.id, row.currentVersionId))
        .run();
    }
    ...
    this.deps.logger.info('cora.evidence.lifecycle', { workspaceId: args.workspaceId, externalId: args.externalId, state: args.state, versions: ids.length });
    return { sourceObjectId: row.id, invalidatedVersionIds: ids };
  }

  getVersion(workspaceId: string, versionId: string) {
    return this.db.select().from(schema.coraEvidenceVersions)
      .where(and(eq(schema.coraEvidenceVersions.workspaceId, workspaceId), eq(schema.coraEvidenceVersions.id, versionId)))
      .get() ?? null;
  }
  ...
  applyAclChange(workspaceId: string, versionId: string, acl: AccessPolicy): void {
    ...
    if (acl.fidelity === 'unavailable' || acl.mode === 'unknown') {
      ...
      if (version) {
        const object = this.db.select().from(schema.coraSourceObjects)
          .where(eq(schema.coraSourceObjects.id, version.sourceObjectId)).get();
        if (object) {
          const doc = this.db.select().from(schema.kbDocuments)
            .where(and(
    ...
    this.deps.logger.info('cora.evidence.acl_changed', { workspaceId, versionId, fidelity: acl.fidelity });
  }
  ...
  getObjectByExternalId(workspaceId: string, connectionId: string, externalId: string) {
    return this.db.select().from(schema.coraSourceObjects)
      .where(and(
        eq(schema.coraSourceObjects.workspaceId, workspaceId),
        eq(schema.coraSourceObjects.connectionId, connectionId),
        eq(schema.coraSourceObjects.externalId, externalId),
      ))
      .get() ?? null;
  }
  ...
  getProvenance(workspaceId: string, versionId: string): { sourceType: string; origin: string } | null {
    ...
    const object = this.db.select().from(schema.coraSourceObjects)
      .where(eq(schema.coraSourceObjects.id, version.sourceObjectId))
      .get();
  ...
  listRecent(workspaceId: string, limit = 50) {
    return this.db.select().from(schema.coraEvidenceVersions)
      .where(eq(schema.coraEvidenceVersions.workspaceId, workspaceId))
      .orderBy(desc(schema.coraEvidenceVersions.createdAt))
