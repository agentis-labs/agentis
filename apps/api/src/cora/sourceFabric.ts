/**
 * CORA Source Fabric — connections + restartable synchronization (RFC §7).
 *
 ...
 * every batch commits its cursor + checkpoint to cora_sync_runs before the
...
export class CoraSourceFabric {
  ...
  createConnection(args: {
    ...
    const id = randomUUID();
    this.db.insert(schema.coraSourceConnections).values({
      id,
  ...
  getConnection(workspaceId: string, connectionId: string) {
    return this.db.select().from(schema.coraSourceConnections)
      .where(and(
        eq(schema.coraSourceConnections.workspaceId, workspaceId),
        eq(schema.coraSourceConnections.id, connectionId),
      ))
      .get() ?? null;
  }
  ...
  listConnections(workspaceId: string) {
    return this.db.select().from(schema.coraSourceConnections)
      ...
      .orderBy(desc(schema.coraSourceConnections.createdAt))
      .all();
  }
  ...
  updateConnection(workspaceId: string, connectionId: string, patch: Partial<{
    ...
    this.db.update(schema.coraSourceConnections)
      .set(sets)
      .where(and(
        eq(schema.coraSourceConnections.workspaceId, workspaceId),
        eq(schema.coraSourceConnections.id, connectionId),
  ...
  async runSync(args: {
    ...
    if (!connection) throw new Error(`Unknown CORA source connection: ${args.connectionId}`);
    ...
    this.db.insert(schema.coraSyncRuns).values({
      id: syncRunId,
      workspaceId: args.workspaceId,
    ...
    try {
      ...
      if (source.resolveAcl && args.mode !== 'sample') {
        try {
          ...
          this.deps.logger.warn('cora.sync.acl_failed', {
            connectionId: args.connectionId,
      ...
      if (source.resolvePrincipals && this.deps.identity && args.mode !== 'sample') {
        try {
          ...
          this.deps.logger.warn('cora.sync.principals_failed', {
            connectionId: args.connectionId,
            error: error instanceof Error ? error.message : String(error),
          });
        }
      ...
      this.db.update(schema.coraSourceConnections)
        ...
        .where(eq(schema.coraSourceConnections.id, args.connectionId))
        .run();
      this.deps.logger.info('cora.sync.completed', { workspaceId: args.workspaceId, connectionId: args.connectionId, mode: args.mode, objectsSeen, versionsCreated, deletions });
      ...
      this.db.update(schema.coraSourceConnections)
        ...
        .where(eq(schema.coraSourceConnections.id, args.connectionId))
        .run();
      this.deps.logger.warn('cora.sync.failed', { workspaceId: args.workspaceId, connectionId: args.connectionId, error: message });
  ...
  async discoverConnectionScopes(workspaceId: string, connectionId: string) {
    ...
    if (!connection) throw new Error(`Unknown CORA source connection: ${connectionId}`);
    const source = this.sources.get(connection.sourceType);
    if (!source) throw new Error(`No KnowledgeSource registered for type: ${connection.sourceType}`);
  ...
  armWebhook(workspaceId: string, connectionId: string): { path: string } {
    ...
    if (!secret) {
      ...
      this.db.update(schema.coraSourceConnections)
        ...
        .where(eq(schema.coraSourceConnections.id, connectionId))
        .run();
    }
    return { path: `/v1/cora-webhooks/${connectionId}/${secret}` };
  }
  ...
  handleWebhookHint(connectionId: string, secret: string): { accepted: boolean } {
    const connection = this.db.select().from(schema.coraSourceConnections)
      .where(eq(schema.coraSourceConnections.id, connectionId))
      .get();
  ...
  listSyncRuns(workspaceId: string, connectionId: string, limit = 20) {
    return this.db.select().from(schema.coraSyncRuns)
      .where(and(
        eq(schema.coraSyncRuns.workspaceId, workspaceId),
        eq(schema.coraSyncRuns.connectionId, connectionId),
      ))
      .orderBy(desc(schema.coraSyncRuns.createdAt))
      .limit(limit)
      .all();
  }
  ...
  private latestRun(workspaceId: string, connectionId: string) {
    return this.db.select().from(schema.coraSyncRuns)
      .where(and(
        eq(schema.coraSyncRuns.workspaceId, workspaceId),
        eq(schema.coraSyncRuns.connectionId, connectionId),
        eq(schema.coraSyncRuns.status, 'completed'),
      ))
      .orderBy(desc(schema.coraSyncRuns.createdAt))
      .limit(1)
      .get() ?? null;
