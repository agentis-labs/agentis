/**
 * CORA Investigations — bounded organizational Feynman loop (RFC §11.2).
 *
...
export class CoraInvestigationService {
  ...
  list(workspaceId: string) {
    return this.db.select().from(schema.coraInvestigations)
      .where(eq(schema.coraInvestigations.workspaceId, workspaceId))
      .orderBy(desc(schema.coraInvestigations.createdAt))
      .limit(50)
      .all();
  }
  ...
  get(workspaceId: string, id: string) {
    return this.db.select().from(schema.coraInvestigations)
      .where(and(eq(schema.coraInvestigations.workspaceId, workspaceId), eq(schema.coraInvestigations.id, id)))
      .get() ?? null;
  }
  ...
  async run(args: { workspaceId: string; question: string; requester?: Record<string, unknown> }): Promise<{ id: string; status: string; grounding: number }> {
    ...
    this.db.insert(schema.coraInvestigations).values({
      id,
    ...
    try {
      ...
      this.db.update(schema.coraInvestigations)
        .set({ status: 'failed', completedAt: new Date().toISOString(), gapsJson: [message] })
        .where(eq(schema.coraInvestigations.id, id))
        .run();
      this.deps.logger.warn('cora.investigation.failed', { workspaceId: args.workspaceId, id, error: message });
  ...
  private finish(workspaceId: string, id: string, result: {
    ...
    this.db.update(schema.coraInvestigations)
      ...
      .where(eq(schema.coraInvestigations.id, id))
      .run();
    this.deps.logger.info('cora.investigation.finished', { workspaceId, id, status: result.status, grounding: result.grounding });
