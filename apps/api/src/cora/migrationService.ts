/**
 * CORA Migration Engine — observe → score → trust-gated candidacy (RFC §18).
 *
...
export class CoraMigrationService {
  ...
  observe(args: {
    ...
    const existing = this.db.select().from(schema.coraMigrationCandidates)
      .where(and(
        eq(schema.coraMigrationCandidates.workspaceId, args.workspaceId),
        eq(schema.coraMigrationCandidates.title, args.title),
      ))
      .get();
    const now = new Date().toISOString();
    if (existing) {
      this.db.update(schema.coraMigrationCandidates)
        .set({
        ...
        .where(eq(schema.coraMigrationCandidates.id, existing.id))
        .run();
      return this.get(args.workspaceId, existing.id)!;
    }
    const id = randomUUID();
    this.db.insert(schema.coraMigrationCandidates).values({
      id,
  ...
  get(workspaceId: string, candidateId: string) {
    return this.db.select().from(schema.coraMigrationCandidates)
      .where(and(eq(schema.coraMigrationCandidates.workspaceId, workspaceId), eq(schema.coraMigrationCandidates.id, candidateId)))
      .get() ?? null;
  }

  list(workspaceId: string) {
    return this.db.select().from(schema.coraMigrationCandidates)
      .where(eq(schema.coraMigrationCandidates.workspaceId, workspaceId))
      .orderBy(desc(schema.coraMigrationCandidates.updatedAt))
      .all();
  }
  ...
  trustGate(workspaceId: string, candidateId: string): TrustGateResult {
    ...
    const claims = this.db.select().from(schema.coraClaims)
      .where(and(eq(schema.coraClaims.workspaceId, workspaceId), inArray(schema.coraClaims.id, claimIds)))
      .all();
  ...
  evaluate(workspaceId: string, candidateId: string): { status: string; gate: TrustGateResult; recommendedTarget: string } {
    ...
    this.db.update(schema.coraMigrationCandidates)
      ...
      .where(eq(schema.coraMigrationCandidates.id, candidateId))
      .run();
    this.deps.logger.info('cora.migration.evaluated', { workspaceId, candidateId, status, gatePassed: gate.passed });
  ...
  shadow(workspaceId: string, candidateId: string) {
    ...
    this.db.update(schema.coraMigrationCandidates)
      ...
      .where(eq(schema.coraMigrationCandidates.id, candidateId))
      .run();
    this.deps.logger.info('cora.migration.shadowed', { workspaceId, candidateId, coverage, verdict: comparison.verdict });
    return this.get(workspaceId, candidateId);
  }
   ...
   * canvas. CORA never activates automation itself (§15.6 `build` contract).
   */
  approve(workspaceId: string, candidateId: string, ownerUserId: string) {
    ...
    this.db.update(schema.coraMigrationCandidates)
      ...
      .where(eq(schema.coraMigrationCandidates.id, candidateId))
      .run();
    this.deps.logger.info('cora.migration.approved', { workspaceId, candidateId, workflowId });
