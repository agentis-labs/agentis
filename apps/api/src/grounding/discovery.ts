/**
 * Grounding Onboarding — read-only discovery + the three-moment quickstart
...
import type { GroundingSourceFabric } from './sourceFabric.js';
import type { GroundingContextComposer } from './contextComposer.js';
import type { AgentGrantMode, GroundingDiscoveryResult, GroundingSourceCandidate, LearningPlanStage } from './types.js';
...
export interface DiscoveryDeps {
  db: AgentisSqliteDb;
  logger: Logger;
  fabric: GroundingSourceFabric;
  composer: GroundingContextComposer;
}

export class GroundingDiscoveryService {
  ...
  discover(workspaceId: string): GroundingDiscoveryResult {
    ...
    const detectedSources: GroundingSourceCandidate[] = [];
    ...
    const result: GroundingDiscoveryResult = {
      workspaceId,
  ...
  launch(args: {
    ...
    let profile = this.db.select().from(schema.groundingOwnerProfiles)
      .where(eq(schema.groundingOwnerProfiles.workspaceId, args.workspaceId)).get();
    if (!profile) {
      const id = randomUUID();
      this.db.insert(schema.groundingOwnerProfiles).values({
        id,
      ...
      profile = this.db.select().from(schema.groundingOwnerProfiles).where(eq(schema.groundingOwnerProfiles.id, id)).get()!;
    } else {
    ...
    let plan = this.db.select().from(schema.groundingLearningPlans)
      .where(eq(schema.groundingLearningPlans.workspaceId, args.workspaceId)).get();
    if (!plan) {
      const id = randomUUID();
      this.db.insert(schema.groundingLearningPlans).values({
        id,
        workspaceId: args.workspaceId,
        ownerUserId: args.ownerUserId ?? null,
      ...
      plan = this.db.select().from(schema.groundingLearningPlans).where(eq(schema.groundingLearningPlans.id, id)).get()!;
    } else {
      const merged = [...new Set([...(plan.sourceConnectionIdsJson as string[] ?? []), ...connectionIds])];
      this.db.update(schema.groundingLearningPlans)
        .set({ sourceConnectionIdsJson: merged, updatedAt: now })
        .where(eq(schema.groundingLearningPlans.id, plan.id))
        .run();
    }
    this.deps.logger.info('grounding.onboarding.launched', { workspaceId: args.workspaceId, connections: connectionIds.length });
    return { profileId: profile.id, learningPlanId: plan.id, connectionIds };
  }

  getProfile(workspaceId: string) {
    return this.db.select().from(schema.groundingOwnerProfiles)
      .where(eq(schema.groundingOwnerProfiles.workspaceId, workspaceId)).get() ?? null;
  }

  getLearningPlan(workspaceId: string) {
    return this.db.select().from(schema.groundingLearningPlans)
      .where(eq(schema.groundingLearningPlans.workspaceId, workspaceId)).get() ?? null;
  }

  /**
   * Workspace Brain V1 path: the internal Agentis-native source is automatic.
   * This idempotently provisions the profile, learning plan, and native
   * connection so the Brain learns from the workspace without a separate
   * onboarding surface.
   */
  ensureInternalWorkspace(args: {
    workspaceId: string;
    ownerUserId?: string | null;
  }) {
    const hadProfile = Boolean(this.getProfile(args.workspaceId));
    const hadPlan = Boolean(this.getLearningPlan(args.workspaceId));
    const existingConnection = this.deps.fabric
      .listConnections(args.workspaceId)
      .find((connection) => connection.sourceType === 'agentis_native');
    const launched = this.launch({
      workspaceId: args.workspaceId,
      ownerUserId: args.ownerUserId ?? null,
      acceptSources: ['agentis_native'],
    });
    return {
      ...launched,
      created: !hadProfile || !hadPlan || !existingConnection,
      needsSync: !existingConnection || !existingConnection.lastSyncAt,
    };
  }
  ...
  private persistProfileDraft(workspaceId: string, result: GroundingDiscoveryResult): void {
