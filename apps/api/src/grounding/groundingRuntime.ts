/**
 * Grounding Runtime — the continuous learning tick (RFC §10.8 Learning Plan).
 *
...
import type { GroundingExtractionService } from './extractionService.js';
import type { GroundingModelService } from './modelService.js';
import type { GroundingDiscoveryService } from './discovery.js';

const DEFAULT_INTERVAL_MS = 5 * 60_000;

export interface GroundingRuntimeDeps {
  db: AgentisSqliteDb;
  logger: Logger;
  fabric: GroundingSourceFabric;
  extraction: GroundingExtractionService;
  model: GroundingModelService;
  discovery: GroundingDiscoveryService;
  intervalMs?: number;
}

export class GroundingRuntime {
  #timer: ReturnType<typeof setInterval> | null = null;
  #ticking = false;

  constructor(private readonly deps: GroundingRuntimeDeps) {}
  ...
  start(): void {
    ...
    this.deps.logger.info('grounding.runtime.started', { intervalMs: interval });
  }
  ...
  async tick(): Promise<void> {
    ...
    this.#ticking = true;
    try {
      const plans = this.deps.db.select().from(schema.groundingLearningPlans)
        .where(eq(schema.groundingLearningPlans.status, 'active'))
        .all();
      for (const plan of plans) {
        await this.tickWorkspace(plan.workspaceId);
      }
    } catch (error) {
      this.deps.logger.warn('grounding.runtime.tick_failed', {
