/**
 * Grounding Extraction — evidence → claims (RFC §10.1 `extract`/`resolve` stages).
 *
...
import type { GroundingMigrationService } from './migrationService.js';
...
export class GroundingExtractionService {
  ...
  async extractPending(workspaceId: string): Promise<ExtractionOutcome> {
    const pending = this.db.select().from(schema.groundingEvidenceVersions)
      .where(and(
        eq(schema.groundingEvidenceVersions.workspaceId, workspaceId),
        eq(schema.groundingEvidenceVersions.extractionStatus, 'ready'),
      ))
      .orderBy(asc(schema.groundingEvidenceVersions.createdAt))
      .limit(MAX_BATCH)
      .all();
    ...
    for (const version of pending) {
      ...
      try {
        ...
        this.db.update(schema.groundingEvidenceVersions)
          ...
          .where(eq(schema.groundingEvidenceVersions.id, version.id))
          .run();
        ...
        this.db.update(schema.groundingEvidenceVersions)
          ...
          .where(eq(schema.groundingEvidenceVersions.id, version.id))
          .run();
        this.deps.logger.warn('grounding.extract.failed', {
          workspaceId,
    ...
    if (outcome.versionsProcessed > 0) {
      this.deps.logger.info('grounding.extract.completed', { workspaceId, ...outcome });
    }
    return outcome;
  }
  ...
  private countRunsByTitle(workspaceId: string, normalizedTitle: string): number {
    const rows = this.db.select().from(schema.groundingClaims)
      .where(and(
        eq(schema.groundingClaims.workspaceId, workspaceId),
        inArray(schema.groundingClaims.predicate, ['run_succeeded', 'run_failed']),
      ))
      .all();
