/**
 * Grounding Identity — entities, source principals, and cross-source identity
 * links (RFC §9.1–§9.2).
 *
...
export class IdentityService {
  ...
  upsertEntity(args: {
    ...
    const existing = this.db.select().from(schema.groundingEntities)
      .where(and(
        eq(schema.groundingEntities.workspaceId, args.workspaceId),
        eq(schema.groundingEntities.kind, args.kind),
        eq(schema.groundingEntities.name, args.name),
      ))
      .get();
    if (existing) {
      if (args.aliases?.length || args.attributes) {
        const aliases = [...new Set([...(existing.aliasesJson as string[] ?? []), ...(args.aliases ?? [])])];
        this.db.update(schema.groundingEntities)
          .set({
            aliasesJson: aliases,
            attributesJson: { ...(existing.attributesJson as Record<string, unknown>), ...(args.attributes ?? {}) },
            updatedAt: new Date().toISOString(),
          })
          .where(eq(schema.groundingEntities.id, existing.id))
          .run();
      }
      return this.getEntity(args.workspaceId, existing.id)!;
    }
    const id = randomUUID();
    this.db.insert(schema.groundingEntities).values({
      id,
  ...
  getEntity(workspaceId: string, entityId: string) {
    return this.db.select().from(schema.groundingEntities)
      .where(and(eq(schema.groundingEntities.workspaceId, workspaceId), eq(schema.groundingEntities.id, entityId)))
      .get() ?? null;
  }

  listEntities(workspaceId: string, options: { kind?: string; limit?: number } = {}) {
    const conditions = [eq(schema.groundingEntities.workspaceId, workspaceId), eq(schema.groundingEntities.status, 'active')];
    if (options.kind) conditions.push(eq(schema.groundingEntities.kind, options.kind));
    return this.db.select().from(schema.groundingEntities)
  ...
  upsertPrincipal(args: { workspaceId: string; connectionId: string; principal: SourcePrincipalInput }) {
    const existing = this.db.select().from(schema.groundingSourcePrincipals)
      .where(and(
        eq(schema.groundingSourcePrincipals.workspaceId, args.workspaceId),
        eq(schema.groundingSourcePrincipals.connectionId, args.connectionId),
        eq(schema.groundingSourcePrincipals.externalPrincipalId, args.principal.externalPrincipalId),
      ))
      .get();
    if (existing) {
      this.db.update(schema.groundingSourcePrincipals)
        .set({
        ...
        .where(eq(schema.groundingSourcePrincipals.id, existing.id))
        .run();
      const updated = this.db.select().from(schema.groundingSourcePrincipals).where(eq(schema.groundingSourcePrincipals.id, existing.id)).get()!;
      this.tryDeterministicLink(args.workspaceId, updated);
      return updated;
    }
    const id = randomUUID();
    this.db.insert(schema.groundingSourcePrincipals).values({
      id,
      workspaceId: args.workspaceId,
    ...
    const created = this.db.select().from(schema.groundingSourcePrincipals).where(eq(schema.groundingSourcePrincipals.id, id)).get()!;
  ...
  private tryDeterministicLink(workspaceId: string, principal: { id: string; email: string | null; displayName: string | null }): void {
    ...
    if (!email) return;
    // Already linked?
    const existingLink = this.db.select().from(schema.groundingIdentityLinks)
      .where(and(
        eq(schema.groundingIdentityLinks.workspaceId, workspaceId),
        eq(schema.groundingIdentityLinks.principalId, principal.id),
        eq(schema.groundingIdentityLinks.status, 'active'),
      ))
      .get();
    if (existingLink) return;
    // Another principal with the same email already linked to an entity?
    const peers = this.db.select().from(schema.groundingSourcePrincipals)
      .where(and(eq(schema.groundingSourcePrincipals.workspaceId, workspaceId), eq(schema.groundingSourcePrincipals.email, principal.email)))
      .all()
      .filter((p) => p.id !== principal.id);
    for (const peer of peers) {
      const peerLink = this.db.select().from(schema.groundingIdentityLinks)
        .where(and(
          eq(schema.groundingIdentityLinks.workspaceId, workspaceId),
          eq(schema.groundingIdentityLinks.principalId, peer.id),
          eq(schema.groundingIdentityLinks.status, 'active'),
        ))
        .get();
      if (peerLink) {
  ...
  createLink(args: {
    ...
    this.db.insert(schema.groundingIdentityLinks).values({
      id,
      workspaceId: args.workspaceId,
    ...
    return this.db.select().from(schema.groundingIdentityLinks).where(eq(schema.groundingIdentityLinks.id, id)).get()!;
  }

  /** The identity review queue (RFC §15.4). */
  listReviewQueue(workspaceId: string) {
    return this.db.select().from(schema.groundingIdentityLinks)
      .where(and(eq(schema.groundingIdentityLinks.workspaceId, workspaceId), eq(schema.groundingIdentityLinks.status, 'review')))
      .orderBy(desc(schema.groundingIdentityLinks.createdAt))
      .all();
  }

  resolveLink(workspaceId: string, linkId: string, decision: 'approve' | 'reject', reviewedBy: string) {
    const now = new Date().toISOString();
    this.db.update(schema.groundingIdentityLinks)
      .set({
      ...
      .where(and(eq(schema.groundingIdentityLinks.workspaceId, workspaceId), eq(schema.groundingIdentityLinks.id, linkId)))
      .run();
    return this.db.select().from(schema.groundingIdentityLinks).where(eq(schema.groundingIdentityLinks.id, linkId)).get() ?? null;
  }
