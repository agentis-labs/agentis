...
import type { KnowledgeHit } from './appIntelligence.js';
import type { EvaluatorExample, WorkflowBaselineSnapshot } from './appIntelligence.js';
...
/** Where to search (precedence order: app → workspace → cross-app). */
export type RetrievalScope = 'run' | 'app' | 'workspace' | 'cross_app';
...
export interface RetrievalParams {
  workspaceId: string;
  /** Optional app scope. When set, app-local results are ranked higher. */
  appId?: string;
  workflowId?: string;
  runId?: string;
  agentId?: string;
