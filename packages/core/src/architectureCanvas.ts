...
export const PROPOSED_PLAN_RE = /<proposed_plan>\s*([\s\S]*?)\s*<\/proposed_plan>/i;
export const ARCHITECTURE_CANVAS_RE = /<architecture_canvas>\s*([\s\S]*?)\s*<\/architecture_canvas>/i;
...
export function planMissingArchitectureCanvas(text: string): boolean {
  ...
  const architectureMatch = text.match(ARCHITECTURE_CANVAS_RE);
  return parseArchitectureCanvas(architectureMatch?.[1] ?? null) === null;
}
