...
### Knowledge Base vs Brain (two distinct stores — stop conflating them)

- **Knowledge Base** = RAG over uploaded/ingested **documents** (`KnowledgeBaseService`). Read in a workflow via the `knowledge` node ("Knowledge search"). For grounding an agent in *your docs*.
- **Brain** = the agents' learned **memory** (`MemoryStore`/`EpisodicMemoryStore`, one `memory_episodes` table; workspace mind = `scopeId null`, individual mind = `scopeId agentId`). It fills **automatically** from chat turns + completed runs through the FormationJudge (see `project_brain_chat_capture`), and agents write to it on purpose via `agentis.memory.*` tools.
- **Writing knowledge/memory is NOT a graph node.** `knowledge_ingest` ("Save to Brain") was mislabeled (it wrote to the Knowledge Base, not the Brain) and is now `@deprecated` for authoring: removed from the node palette + synthesis catalog, still executed for back-compat. New writes go through auto-capture or the `agentis.knowledge.write` tool.
...
## D39 — Agentic App Interface rebuild (control plane + open block registry)

**Problem.** An App was modeled as *identity + surfaces* with workflows attached only by `workflows.appId` — there was no model or API for an App's workflows (purpose / order / dependencies / trigger / status), so a multi-workflow App was a bag of dead cards with no run/order/explain control. And the surface renderer (`ViewRenderer.tsx`) was a 2101-line, 52-case `switch` with silent data-binding failures and a closed block set.

**E0 — App workflow control plane.** Per-workflow App metadata (`purpose`, `order`, `dependsOn[]`, `enabled`) lives on the workflow's own `settings.appBinding` (no migration — a workflow belongs to exactly one App). New routes on `apps.ts`: `GET /:id/workflows` (binding + derived `triggerKind` + `lastRun`, sorted by order), `POST /:id/workflows/:wid/run` (ownership-guarded, reuses `startPublishedWorkflow` — execution is **not** forked), `PATCH /:id/workflows/:wid/binding`. Types in `packages/core/src/types/app.ts` (`appWorkflowBindingSchema`, `AppWorkflowSummary`).

**E1 — visible binding failures + design-var spacing.** An unresolved `{$bind}`/`{$state}`/`{$row}` rendered as silent-empty (or a bare `—` on a Metric), so a broken App looked merely empty. Now `resolveDisplay()` surfaces a visible `⚠ unbound: <path>` marker on the scalar leaves (Text/Heading/Markdown/Badge/Metric/KPIStrip). Separately, Stack/Row/Grid hardcoded a binary `8/12px` gap while KPIStrip honored the design language's `--s-gap`; all layout now honors `--s-gap`.

**E2 — open block registry (the seam).** The 52-case `switch` is replaced by a dispatch through `blocks/registry.ts` (`registerBlock`/`getBlock`/`listBlockKinds`). Built-ins register at module load; an agent/plugin/workspace can register or override a kind via the same public API; an unregistered kind renders a visible `UnknownBlock` (not silent `null`). Blocks recurse through `BlockContext.renderChild` instead of importing the renderer, so a block can live in any module (proven by a test that registers a block from outside `ViewRenderer`).

**Gotcha (load-bearing).** `ViewNode = ViewNodeBase & { style? }` (an **intersection**, view.ts:228), so generic `Extract<ViewNode, { type: K }>` collapses to `never` — and even concrete `Extract` fails for the enum-typed members (`Row|Grid`, `Card|Section`). Block renderers therefore narrow `node` with a **control-flow check** (`if (node.type !== 'X') return null`), which is what the old `switch` relied on. Don't reach for `Extract` here.

**Deferred — stable node ids.** The plan bundled "stable view-tree ids" into E2, but the entire mutation/selection layer (`viewTree.ts`, `SurfaceEditContext`) is index-path-based and shared by `SurfaceCanvas` + `AppEditorPage`; ids reach into the core `ViewNode` schema + builder + persistence. That is a separate change, not part of the rendering seam, and was deliberately not bundled.

**E3 — monochrome design system + dashboard-first generator.** Operator signed off on a dashboard-first direction but changed the brand accent emerald → **white/ghost-white** (Linear/Vercel monochrome; "colors/gradients are fine, the highlights are white"). `designLanguage.ts` `MONOCHROME_BASE_VARS` is merged into every design language via `resolveDesign` — re-points `--color-accent`/`--color-success`/`--shadow-glow` to ghost-white, **scoped to the app surface root** (platform/home keep emerald). New `--color-on-accent` token (white globally, `#08090b` in apps) + Tailwind `on-accent` fixes 6 filled buttons that were white-on-white. The **Workflow Control Plane** ships as a real block: new ViewNode kind `WorkflowControl` (app-scoped, reads the E0 endpoint, run button per row), registered on the E2 seam; the **generator is dashboard-first** (`surfaceGenerator` SYSTEM/PATCH prompts + `chatToolCatalog` `ui.render` + `genui.ts` scaffold all lead an App overview with KPIs + WorkflowControl, forms behind a Tab). Tables are now **sortable** (numeric-aware header click). Charts read the token accent (ghost-white series). **E0 regression fixed:** E0's rich `GET /:id/workflows` had shadowed a pre-existing bare-`string[]` route, breaking `AppEditorPage`; the dead route is deleted and the client/page moved to the rich `AppWorkflowSummary[]` shape. Deferred polish (incremental, blocks are functional + on-brand): a Recharts swap, table filter/pagination, forms restyle, app-shell. Verify: tsc clean (core/api/web); `ViewRenderer.test.tsx` 18/18, `apps.test.ts` 26/26, surfaceGenerator 8/8, genuiAudit 9/9.

**Status.** `apps/web` tsc clean; `ViewRenderer.test.tsx` 15/15 (adds binding-marker, design-var, dispatch, unknown-fallback, and runtime-registration tests); E0 route tests in `apps/api/tests/routes/apps.test.ts`. Physically relocating the co-located view components into `blocks/<Kind>.tsx` is a safe incremental follow-up the seam now enables.
