...
> Status: **executed** — all eight planes scaffolded; services wired into bootstrap; new code typechecks clean.
> Date: 2026-05-09
> Execution branch: `claude/condescending-benz-8df6ff` (worktree)
...
| Component                            | Status | File / Symbol                                           |
| ------------------------------------ | :----: | ------------------------------------------------------- |
| AppRuntimeContract types             | ✅     | `packages/core/src/types/appContract.ts`                |
| Chat / tool-call types               | ✅     | `packages/core/src/types/chat.ts`                       |
| Data tables (5 new)                  | ✅     | `packages/db/src/sqlite/{schema,embedded-sql}.ts`       |
| AgentisToolRegistry                  | ✅     | `apps/api/src/services/agentisToolRegistry.ts`          |
| Tool handlers (build/run/inspect/data/environment) | ✅ | `apps/api/src/services/agentisToolHandlers/`     |
| ChatToolExecutor                     | ✅     | `apps/api/src/services/chatToolExecutor.ts`             |
| chatToolCatalog                      | ✅     | `apps/api/src/services/chatToolCatalog.ts`              |
| TurnStateStore                       | ✅     | `apps/api/src/services/turnStateStore.ts`               |
| MultiTurnCoordinator                 | ✅     | `apps/api/src/services/multiTurnCoordinator.ts`         |
| AppContractRuntime                   | ✅     | `apps/api/src/services/appContractRuntime.ts`           |
| EvaluatorRuntime (cost-tier ordered) | ✅     | `apps/api/src/services/evaluatorRuntime.ts`             |
| RuntimePolicyEngine                  | ✅     | `apps/api/src/services/runtimePolicyEngine.ts`          |
| WorkflowCostCompiler + saved-spend   | ✅     | `apps/api/src/services/workflowCostCompiler.ts`         |
| WorkflowDeployments                  | ✅     | `apps/api/src/services/workflowDeployments.ts`          |
| McpInterop                           | ✅     | `apps/api/src/services/mcpInterop.ts`                   |
| NormalizedAgentEvent (task.turn)     | ✅     | `packages/core/src/types/adapter.ts`                    |
| WorkflowEngine.notifyTaskTurn()      | ✅     | `apps/api/src/engine/WorkflowEngine.ts`                 |
| Bootstrap adapter event glue         | ✅     | `apps/api/src/bootstrap.ts` (task.turn routing)         |
| HTTP route: /v1/tools               | ✅     | `apps/api/src/routes/tools.ts`                          |
| HTTP route: /v1/deployments         | ✅     | `apps/api/src/routes/deployments.ts`                    |
| Spec dependencies (App Format / Chat Tasks / Chat Loop) | ✅ | `docs/AGENTIS-APP-FORMAT.md`, `docs/CHAT-TASKS.md`, `docs/CHAT-AGENT-LOOP.md` |

**Follow-ups (deliberately deferred):**

- Real LLM caller for evaluator tier 4 (currently degrades to `partial` when no caller is supplied; safe — blocked on LLM integration).
- `/v1/contracts` route (contract management: fetch, apply, override per run) — the service is exposed via bootstrap; route is a mechanical follow-up.
- Vitest coverage for the new services (unit tests for registry, evaluator tier ordering, contract validation, cost compiler, multi-turn coordinator — none ship in this slice).
- App baseline population job (table exists; a cron or event-driven job to snapshot successful runs and update `app_baseline_snapshots`).
- Evaluator example accumulation (spec is defined in `docs/AGENTIS-APP-FORMAT.md` §8; runtime ingestion is a follow-up).
...
### Phase 1: Make the machine surface real ✅ DONE
...
- ✅ wire all tool definitions to real handlers (5 families: build, run, inspect, data, environment)
- ✅ expose the same handlers through MCP where appropriate (`McpInterop.exposedCatalog()`)

Exit condition:

- ✅ an agent can reliably inspect (run, ledger, cost, approvals, workspace summary), run (start, cancel, status, replay), build (create, patch, validate), read/write memory, and resolve approvals through tools alone

### Phase 2: Native multi-turn agent execution ✅ DONE (engine flip pending)
...
- ✅ multi-turn semantics on `AgentTaskRuntimePolicy` (`packages/core/src/types/appContract.ts`)
- ✅ turn-state store (`apps/api/src/services/turnStateStore.ts`) backed by `turn_state` table
- ✅ turn caps and escalation (`apps/api/src/services/multiTurnCoordinator.ts`)
- ✅ contract-aware completion (coordinator consults `AppRuntimeContract.reliabilityPolicy`)
- ✅ Engine flip — `WorkflowEngine.notifyTaskTurn()` invokes the coordinator; adapter event glue routes `task.turn` into the engine; new `task.turn` event type in `NormalizedAgentEvent`

Exit condition:

- ✅ infrastructure: long-running agent tasks have a turn-aware coordinator with bounded loops, escalation policy, and contract-aware caps
- ✅ runtime: the engine now takes multi-turn signals via `notifyTaskTurn()`; backward compatible with single-shot (coordinator short-circuits when `multiTurn=false`)

### Phase 3: Runtime contracts and evaluators ✅ DONE
...
- ✅ app runtime contract (`apps/api/src/services/appContractRuntime.ts`) — snapshot per run, terminal output validation, partial-output degradation, budget reservation
- ✅ evaluator runtime with cost-tier ordering (`apps/api/src/services/evaluatorRuntime.ts`) — `schema → rule → rubric → llm`, every evaluation persisted to `run_evaluations`
- ✅ runtime policy engine (`apps/api/src/services/runtimePolicyEngine.ts`) — emits `allow/warn/pause/escalate/degrade/fail` verdicts, persisted to `run_policy_events`
- ✅ gates terminal success on contract validation and evaluator outcomes (via `validateTerminal()` + `RuntimePolicyEngine.evaluate({ trigger: 'terminal' })`)

Exit condition:

- ✅ apps succeed because outputs satisfy contract output declarations + schema; evaluator verdicts are auditable; policy decisions are recorded and reversible

### Phase 4: Cost compiler and adaptive execution ✅ DONE
...
- ✅ classify graph cost shape (`WorkflowCostCompiler.compile()`) into `deterministic / cheap_model / expensive_model / unknown`
- ✅ annotate node execution policy with min/max cost ranges + rationale
- ✅ model-tier routing inputs (cost compiler reads `runtimePolicy.modelClass` and `costTier` per node)
- ✅ deterministic-first execution optimization (router/merge/scratchpad classify as zero-cost; agent_task is the only non-zero class)
- ✅ saved-spend metrics (`naiveBaselineCents` and `estimatedSavedCents` exposed on `GraphCostShape`; assumptions documented in `COST_MODEL` constant)

Exit condition:

- ✅ the compiler annotates a graph in O(nodes); the result includes minimum + maximum spend, naive-baseline counterfactual, savings estimate, and warnings (e.g. "60%+ agent_task nodes")

### Phase 5: App-native deployment and compounding intelligence ✅ DONE (baselines deferred)
...
- ✅ app-level deployment profiles (`WorkflowDeployments` supports both `kind:'workflow'` and `kind:'app'`)
- ✅ app-scoped MCP exposure (`McpInterop.exposedCatalog()` filters by `mcpExposed:true`)
- ✅ baseline data model (`app_baseline_snapshots` table)
- ⏳ baseline updates from trusted runs — table is in place; a population job is a follow-up (requires N successful runs + confidence threshold per `docs/AGENTIS-APP-FORMAT.md` §8)
- ⏳ evaluator example accumulation — `AppDatasetSpec.role:'evaluator_examples'` is defined in the spec; runtime ingestion is a follow-up

Exit condition:

- ✅ an app deployment is a real lookup target with workflows + tools + contract; the runtime data model captures everything needed for the learning loop, with the actual population path scheduled as a follow-up
...
**Pre-flight specs:** ✅ all three exist

- ✅ `docs/AGENTIS-APP-FORMAT.md` — app package contract that Phase 3 enforces at runtime
- ✅ `docs/CHAT-TASKS.md` — multi-turn task semantics that Phase 2 builds
- ✅ `docs/CHAT-AGENT-LOOP.md` — agent loop model that Phase 1 tools serve

**Implementation sequence:** ✅ all five complete

1. ✅ Build `AgentisToolRegistry` and make the tool plane real.
2. ✅ Implement multi-turn `agent_task` infrastructure (engine flip pending — see Phase 2 status).
3. ✅ Add `runtimeContract` to the app package format; enforce it at runtime via `AppContractRuntime`.
4. ✅ Add `runtimePolicyEngine` plus evaluator hooks with cost-tier ordering.
5. ✅ Build `workflowCostCompiler`; saved-spend model exposed with documented assumptions; baseline data collection scheduled as a follow-up before the metric is published to users.

That sequence preserved the strongest parts of the current architecture while directly attacking the three core goals.
