...
### 2026-06-24 — Phase 2 / 3.3 reconciled (one real win; the rest already-satisfied or scale-gated)

- **P2 — model reranker ACTIVATED (was dormant).** `SharedIntelligence.setRerankCompleter(defaultCognitiveCompleter)` wired in bootstrap beside `setFormationCompleter`. The reranker code (`#modelRerank`) already existed; it just was never given a completer, so it was a no-op. Now the top semantic hits are model-reranked before entering a dispatch block. (Tests don't wire a completer → recall suites unchanged; production-only behavior.) **Done.**
- **3.3 — route every scope through the registry: ALREADY SATISFIED.** Abilities route via `resolveAbilityEmbeddingProvider = (ws) => SharedIntelligence.embeddingProvider(ws)` (`bootstrap:786`); the KnowledgeBase semantic path uses the registry resolver; the remaining `new LocalEmbeddingProvider()` instances are documented pre-wiring fallbacks / a sync-lexical default. The only unrouted store is **personalBrain** — account-scoped, intentionally deferred (no account-level embedding config exists; documented `bootstrap:478`). No change.
- **P2 — fold `promoted_patterns`: PREMISE LARGELY FALSE, not done by design.** `IntelligencePromotion.#mirrorToMemoryIfEligible` already promotes recall-worthy patterns (business_rule/recurring_exception/approved_output_pattern/failure_with_fix ≥0.7) into `memory_episodes` — they already get embeddings + lifecycle + recall. Rows that stay in `promoted_patterns` are low-confidence evidence-counters deliberately kept out "to keep memory small". Fully folding would fight that design; **deferred as not-a-win**.
- **P2 — BLOB vectors / `scope_type` column: scale-gated, deferred.** BLOB is a 47-site storage-format change + table-rebuild migrations across ~10 tables, whose only benefit (storage/parse speed) is nil at current scale (brain wiped to ~0 atoms) — pair it with §0.5 ANN, gated on real scale. `scope_type` discriminator has no consumer yet (scopeId NULL/non-null already discriminates). Neither delivers value now; both risk a green, empty brain.
