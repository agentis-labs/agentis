...
- **2026-07-01 — Organ 2 (Intent Contract) v1 SHIPPED** (commit `dc015ae`; typecheck clean, 31 tests green). `intentContract.ts`: (1) APPROVAL INTEGRITY — an approval forced to `true` (`|| true` / constant) before an irreversible action is a hard error (the `deploy-contract-input` bug); (2) CAPABILITY PRESERVATION — a manifest (agent workers, external-fetch steps, integrations, persistence) is derived + stored on `settings.intentManifest` on every build, and an edit that drops load-bearing capabilities (Instagram scout → stub) is flagged against it. Wired into `build_workflow` (check on every build; store manifest; diff on edit). ⚠️ v1: capability regressions are WARNINGS (visible), not hard blocks — the "hard-block-unless-acknowledged" ratchet + NL-derived `must` capabilities are v2. NOT built at that point: Organ 3/4.
- **2026-07-01 — Organ 4 (Failure taxonomy) SHIPPED** (commit `0f0a126` + scope fix `af534dc`; 397-test sweep green). `isResourceFailure` + `resourceBlockerReason` in `WorkflowEngine.ts`; `#failNode` QUARANTINES a RESOURCE failure on an AGENT node (rate/usage limit, quota, auth/captcha wall, transient 5xx/network) — pause with a blocker BEFORE self-heal can edit it, so a rate-limited scout is never rewritten/deleted. Scoped to agent nodes (non-agent keeps retryPolicy/fail semantics).
- **2026-07-01 — Organ 3 (Atomic Evolution / green ratchet) v1 SHIPPED** (commit `5c1a77a`). `build_workflow` rejects an EDIT that INTRODUCES a critical regression the prior graph lacked (a new coupling error or approval bypass) — the workflow never persists a regression (kills whack-a-mole). New workflows exempt. ⚠️ v1 covers couplings + approval-bypass; a broader preflight-diff + explicit-intent-override are v2.
- **ALL 4 ORGANS SHIPPED v1** (Typed Data Plane · Intent Contract · Atomic Evolution · Failure Taxonomy) + the Organ-1 runtime boundary (output adaptation). NOT built: Organ 1-deep (opaque-extension / nested paths), Organ 2 hard-block ratchet (v1 = warnings), the LLM reshape.
