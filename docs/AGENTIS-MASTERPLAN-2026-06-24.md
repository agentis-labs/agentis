...
| 0.3 | Harden CustomView for untrusted UI | ✅ (null-origin sandbox + CSP `connect-src 'none'`; bridge `data.query` gated by `collections` allowlist; **`actions.invoke` now gated by the surface's declared-action allowlist**; frame-source identity + versioned-protocol checks) |
| 0.4 | Provenance gate on installed bundles | ✅ (install permission summary now surfaces `executes-code:<lang>` / `controls-browser` from bundle workflow graphs → explicit ack required) |
...
| 1.3 | Generic per-node `retryPolicy` | ✅ (base-node `retryPolicy`, bounded backoff re-dispatch in `#failNode`, agent-kinds excluded) |
| 1.4 | Durability (loops resume, evaluator bound, durable delegation, checkpoint timeout) | ✅ (checkpoint timeout + durable loop-resume + per-node dispatch ceiling + **durable subflow delegation**: recovery re-binds the parent↔child resume across a restart and resumes immediately if the child finished during downtime — no duplicate child run; restart-simulation test; evaluator-bound subsumed by the dispatch ceiling) |
| 1.5 | Human-in-the-loop inside teams (worker yields) | ✅ (top-level agent_session yields — `await_event` / `sleep_until` / `request_approval` — survive a restart via `#recoverParkedSessions`, re-arming event/timer/approval wakes; synchronous *delegated sub-sessions* park inline by design) |
| 1.6 | Explicit join semantics (merge ↔ parallel) | ✅ (`merge.parallelSourceId` binds the governing parallel; heuristic fallback preserved) |
| 1.7 | Time / aggregation / pagination / human-input primitives | ✅ (wait-until-datetime + `human_input` form + `data_query` pagination + `aggregate_window` cross-run batching, via a generic `__hold` defer-downstream primitive) |
| 2.1 | Turn on native SaaS webhooks | ✅ (`POST /v1/webhooks/connector/:triggerId` → `fireConnectorWebhook` verifies the provider signature for all 14 connectors; idempotent; forged→rejected) |
| 2.2 | Honest connector catalog | ✅ (`connectorReadiness`/`connectorCatalog` classify runnable vs needs-setup; `/v1/integrations` list now tags each entry) |
| 2.3 | MCP as a runtime capability | ✅ (new `mcp` workflow node calls a registered MCP server's tool via the existing `McpToolBridge`) |
| 2.4 | Guided OAuth + low-friction sensing | ◑ (OAuth service exists — `oauthService.ts`; **predicate-failure surfacing already done** — `ListenerRuntime` emits `LISTENER_PREDICATE_FAIL` with the reason. Only prebuilt poll recipes left) |
| 3.x | Brain | 🚫 owner-handled (`docs/brain/BRAIN-SCALE-10X.md`) |
| 4.1 | Datastore depth (aggregation, keyset cursor, closed types) | ✅ (server-side `aggregate` + keyset pagination + opt-in `strict` collections) |
| 4.2 | Fork + Hub publish path | ⬜ |
| 4.3 | In-app agent orchestration | ◑ (operator command already runs the app's workflow [can contain agent/swarm nodes]; new `data_query` + `data_mutate` nodes let workflows read/aggregate/write the app's datastore — closing the workflow↔app-data loop) |
| 4.4 | Customer-facing surface scoping | ⬜ (deferred until a real app needs it) |
| 5.1 | NL composer on the canvas | ✅ (empty-canvas hero composer + per-node "edit in plain English" both POST to the orchestrator send pipeline with `viewportOverride{resourceKind:'workflow',resourceId}`, so the build/patch targets this workflow and its `CANVAS_NODE_PLACED`/`EDGE_CONNECTED` events stream live onto the open canvas — reuses the existing build tool + canvas event consumer) |
| 5.2 | Visual field-picker | ✅ (unit-tested `generateFieldExpression` util — `js` dialect for transform/filter/evaluator, `template` for templated text; `FieldPicker` step→field dropdown wired into 6 forms: Transform, Filter, Agent prompt, HTTP URL, HTTP Body, Browser URL) |
| 5.3 | Demote raw JSON + rename internals | ✅ (run-modal tabs de-jargoned + humanized "Kind" + "What this agent can do"; raw-JSON editor now behind an "Advanced" gear toggle, form is primary) |
| 5.4 | Zero-config model/runtime | ✅ (wizard auto-selects the best *installed* runtime from harness-detect by priority; picker collapsed behind "Configure" with a one-line auto-selected summary; auto-expands only when nothing is detected) |
| 5.5 | Template gallery + output-first | ✅ (create dialog offers starter templates → `appsApi.create({ entryWorkflowGraph })` seeds the entry workflow; backend-tested + preview-verified) |
| 5.6 | Soften the agent-hierarchy on-ramp | ✅ (empty workspace → first-agent mode: role/specialty/reports-to controls hidden, defaults to the orchestrator "the one you talk to" with a friendly note + "Set up a team structure instead" escape that reveals the full controls) |
...
- _2026-06-24_ — **1.4 + 1.5 COMPLETE: durable delegation & worker yields.** (1.4) A subflow parent node stays RUNNING while a child `WorkflowRun` executes; the parent↔child resume binding lived only in `SubflowExecutor`'s in-memory `#pending`, so a restart (a) hung the parent forever and (b) re-dispatched the subflow node, spawning a *duplicate* child. Fix: new `SubflowExecutor.rebind()`/`hasPending()` + engine `#recoverSubflowParent()` — the crash-recovery loop now special-cases `executorType === 'subflow'` (instead of the generic re-dispatch), finds the surviving child via `parentRunId (+ workflow id)`, recovers the parent node id from its own persisted `activeExecutions`, re-binds fresh resume/fail closures over the rehydrated ctx, and if the child already reached terminal during downtime drives the parent resume immediately (the child's terminal transition fired into a then-empty map). No schema migration needed. **Restart-simulation test** (`WorkflowEngine.subflowDurable.test.ts`): persist a mid-subflow crash state + terminal child → fresh engine `recoverInterruptedRuns()` → parent resumes to COMPLETED carrying the child output, **exactly one** child run. (1.5) Confirmed top-level `agent_session` yields (`await_event`/`sleep_until`/`request_approval`) already survive restarts via the pre-existing `#recoverParkedSessions` re-arm pass; delegated sub-sessions resolve inline synchronously by design. **Engine suite 288/288** (no regression from the recovery-path change).
- _2026-06-24_ — **5.1 COMPLETE: NL composer on the canvas.** New `CanvasBuildComposer` with two surfaces. (a) Empty-canvas hero: when `flowNodes.length === 0`, a centered "Describe this workflow" box. (b) Per-node "Edit in plain English" field in the inspector. Both POST to `/v1/conversations/orchestrator/send` with `viewportOverride{ surface:'workflow-canvas', resourceKind:'workflow', resourceId }` — the route's `workflowIdFromViewport` binds the turn to this workflow, so the build tool builds/patches *this* graph and emits `CANVAS_NODE_PLACED`/`CANVAS_EDGE_CONNECTED` scoped to its id, which `WorkflowCanvasPage` already renders live (no refresh). Node-edit names the target step (`id: …`) so the tool patches in place. No new backend — rides the existing build pipeline + canvas event consumer. Web typecheck clean.
- _2026-06-24_ — **5.2 COMPLETE: visual field-picker.** Non-devs no longer hand-write `ctx.nodes.fetch.data[0].name`. New `fieldExpression.ts` generator turns a `(step, field)` pick into the correct reference per dialect: `js` (`ctx.nodes["fetch-1"].items[0]["full name"]` with bracket-escaping of non-identifier/numeric segments) for the sandboxed expression fields (transform/filter/evaluator, which eval against `{ input, ctx:{ trigger, nodes, scratchpad, store } }`) and `template` (`{{nodes.fetch.data.items}}`) for templated text. New `FieldPicker` (step dropdown → field dropdown from `outputKeys`, live expression preview) is layered above the existing field as an assistive affordance — raw expressions/`{{ }}` still work. Wired into 6 forms (Transform, Filter, Agent prompt, HTTP URL, HTTP Body, Browser URL). Generator unit-tested (`fieldExpression.test.ts`, **10/10**); web typecheck clean.
- _2026-06-24_ — **5.6 COMPLETE: softened first-agent on-ramp.** A first-time operator on an empty workspace no longer meets orchestrator/manager/specialist jargon. When `agents.length === 0` (and no preset role), the wizard enters first-agent mode: it defaults to the orchestrator, names it "The Brain", and replaces the Role chooser + HierarchyNotice with one friendly line ("This is your first agent — the one you talk to…") plus a "Set up a team structure instead" link that reveals the full role/hierarchy controls (`showHierarchy`). Once agents exist, the normal controls show as before. Web typecheck clean.
- _2026-06-24_ — **5.4 COMPLETE: zero-config runtime in the agent wizard.** Was hard-defaulting to `claude_code` and showing the full RuntimePicker + Test-runtime up front. Now: on open, after `/v1/harness/detect` resolves, `ADAPTER_PRIORITY` picks the first *installed* (`status:'found'`) harness and selects it automatically; the picker collapses to a one-line summary ("Claude Code · auto-selected (installed). Commission to go.") behind a "Configure" button, and only auto-expands when nothing was detected (so the operator can install/connect one). Commission-anyway path for missing runtimes preserved. Web typecheck clean.
- _2026-06-24_ — **0.3 COMPLETE: CustomView action allowlist.** The CustomView iframe was already strongly isolated (null-origin `sandbox="allow-scripts"`, CSP `default-src 'none'; connect-src 'none'`, frame-source identity check, versioned bridge protocol) and `data.query` was already gated by the node's `collections` allowlist — but `actions.invoke` was ungated, so agent-authored HTML could invoke *any* action name. Added `allowedActions = new Set(surfaceActions.map(a => a.name))`; the bridge now rejects `actions.invoke` for any name the surface doesn't declare (`'action not allowed'`), mirroring the collection gate. Web typecheck clean.
- _2026-06-24_ — **1.7 COMPLETE: cross-run `aggregate_window` node.** Each run appends its input to a persistent `workflow_store` buffer; the node HOLDS (fires no downstream) until the window closes by `maxCount` or `windowMs`, then flushes the batch. Introduced a general `__hold` output primitive — `#completeNode` skips all downstream edges when a node's output sets `__hold:true` (drops itself from each target's required inputs so the run still settles). Collapses a high-frequency trigger into periodic batches (digests/lead-batching). Test `aggregateWindow.test.ts`: 3-count window holds twice → flushes `[{n:1},{n:2},{n:3}]` → resets. **Engine suite 287/287 (no regression from the hot-path `__hold` change).** This finishes 1.7 (wait-until + human_input + pagination + aggregate-window).
- _2026-06-24_ — **5.5 COMPLETE: starter template gallery.** Backend: `create` accepts `entryWorkflowGraph` and seeds the entry workflow from it (route-tested — a 2-node template yields a 2-node workflow). Frontend: new `appTemplates.ts` (Blank / Scheduled digest / Inbound webhook, each a valid graph) + a selectable gallery in the create dialog, passed through `appsApi.create`. **Preview-verified:** all three templates render in the live New-app dialog. apps/web + api typecheck clean; apps-route suite 14/14.
- _2026-06-24_ — **5.3 COMPLETE: raw-JSON editor demoted behind an "Advanced" gear in the inspector** (`ContextInspector.tsx`) — the form is now the primary, sole-default view; JSON is an opt-in escape hatch. apps/web typecheck clean.
- _2026-06-24_ — **Batch: 1.7 pagination + 5.3 de-jargon shipped; 2.4-predicate found already-done.** (1.7) `data_query` gained `paginate`/`maxRows` — follows the keyset cursor internally and returns every matching row (test in `dataQueryNode`, 25 rows across pages). (5.3) De-jargoned the run modal for non-devs (apps/web): tabs `Ledger→Activity log`, `Scratchpad→Working memory`, `Nodes→Steps`; the raw mono "Kind" column is now humanized (`agent_task`→`Agent task`); inspector `HAL runtime requirements`→`What this agent can do`. apps/web typecheck clean; preview bundle healthy (no console errors). (2.4) Predicate-failure surfacing was ALREADY implemented — `ListenerRuntime` emits `LISTENER_PREDICATE_FAIL { reason }` on a miss (audit stale). **Honest remainder:** the bulk of what's left is a frontend program (5.1 NL-on-canvas, 5.2 visual field-picker, 5.4 zero-config model, 5.5 template gallery, 5.6 soften hierarchy, 5.3 demote-raw-JSON, 0.3 CustomView client hardening) + deep async (1.5 worker yields, 1.4 durable delegation, 1.7 cross-run aggregation-window) — each a focused effort, not one-pass work.
- _2026-06-24_ — **Extended batch: 6 tasks shipped (all tested).** (1) **4.1 keyset cursor** — replaced offset pagination with a lexicographic keyset seek (+ stable `id asc` tiebreaker); O(log n), no skip/dupe under concurrent writes. (2) **4.1 closed types** — opt-in `CollectionSchema.strict` rejects unknown keys (`.strict()` vs `.passthrough()`). (3) **2.3 `mcp` node** — workflows call a registered MCP server's tool via the existing `McpToolBridge` (port in engine deps, wired in bootstrap). (4) **1.4 per-node dispatch ceiling** — `#dispatchNode` caps how many times any node may be dispatched per run (default 200, `AGENTIS_NODE_DISPATCH_CEILING`); exceeding it FAILS the run (not a retryable node failure) — a general infinite-cycle backstop that subsumes the murky evaluator-bound concern (caps unbounded LLM/IO cost for any author cycle / runaway self-heal). (5) **`data_query` node** — workflows read/aggregate an App datastore collection (`AppDataPort` in engine deps). (6) **`data_mutate` node** — workflows insert/update/upsert/delete App datastore records. Tests: `appDatastore.queryTypes` (3), `WorkflowEngine.mcpNode` (2), `WorkflowEngine.dispatchCeiling` (1), `WorkflowEngine.dataQueryNode` (2), `WorkflowEngine.dataMutateNode` (2), `connectorCatalog` (4, from 2.2). Core/app/api typecheck clean; aggregate + apps-route + retryPolicy suites green (no regressions). Also found **2.4 OAuth already exists** (`oauthService.ts`, audit was stale).
- _2026-06-24_ — **Phase 4.1 COMPLETE: keyset cursor + closed collection types.** Replaced the offset cursor (O(n) deep, skips/dupes under concurrent writes) with **keyset pagination** — order by the requested sort (or updatedAt desc) + a stable `id asc` tiebreaker, seek past the cursor's encoded row values via a lexicographic predicate. Added opt-in `strict` collections (`CollectionSchema.strict`): writes reject unknown keys (`.strict()`) instead of the permissive `.passthrough()`, so a typed collection is a real guarantee. Test `appDatastore.queryTypes.test.ts` (3): strict rejects/loose accepts unknown keys; keyset walks all rows once across pages in stable order (custom sort + default updatedAt-desc). app+core typecheck clean; aggregate + apps-route suites green (no regression).
- _2026-06-24_ — **Phase 2.2 SHIPPED: honest connector catalog.** The platform advertised ~95 connectors but only the hand-written (7) + per-service-templated (~22) ones run out of the box — the rest fall back to a generic HTTP connector that throws without a URL (a trust trap). New `connectorReadiness(service)` + `connectorCatalog()` in `@agentis/integrations` classify each as `runnable` vs `needs_setup`; the `GET /v1/integrations` list now tags every entry (custom workspace integrations carry their own URLs → runnable). Test `connectorCatalog.test.ts` (4): github/slack hand-written + stripe/notion templated → runnable; salesforce/mongodb → needs_setup; catalog has both classes with display metadata. Integrations package + route typecheck clean; integrations route suite green.
- _2026-06-24_ — **Phase 1.4-partial: durable/idempotent loop resume SHIPPED + a real subflow bug fixed.** `#runLoop` now persists each iteration's result in `_loopState.{completed,failed}` and SKIPS already-settled iterations on re-run, so a crash-recovery re-dispatch (which already re-dispatches in-flight nodes) resumes a loop instead of replaying every iteration and double-firing side effects. **While testing this I found a pre-existing bug:** `SubflowExecutor.findParentByChildRunId` did `key.split(':')`, but a loop iteration's `parentNodeId` is `L:item:N` (contains colons) — the naive split mangled it, the parent was never found, and **loop iterations hung forever**. Fixed to split on the first colon only. Test `WorkflowEngine.loopResume.test.ts` (2): fresh loop runs 3 child runs; a loop seeded with iterations 0&1 completed runs only iteration 2 (1 child) and reuses the persisted results. run_workflow + terminal-transition subflow suites green (no regression). Evaluator-bound + durable-delegation remain (both need careful design — the cycle topology / cross-tick session parking).
- _2026-06-24_ — **Phase 4.1-partial: server-side datastore aggregation SHIPPED.** New `AppDatastore.aggregate(ws, app, collection, { op, field?, groupBy?, filter?, limit? })` runs `count`/`sum`/`avg`/`min`/`max` with optional group-by **in SQL over the full collection** — replacing the client-side grouping of a capped fetch that made Charts/DataBoards silently wrong at scale. Reuses the existing filter/index conditions; bounded group count (≤1000). Test `appDatastore.aggregate.test.ts` (5): count-by, sum-by, ungrouped totals, filtered avg, field-required guard. App package typechecks clean. Keyset cursor + closed-type passthrough (the rest of 4.1) remain.
- _2026-06-24_ — **Phase 0.4 SHIPPED: provenance gate on installed bundles.** `AppPackager.permissionSummary` now scans each bundle workflow graph for executable payloads — a `code` node → `executes-code:<language>`, a `browser` node → `controls-browser` — so an imported third-party bundle that runs code/drives a browser on the INSTALLER's host must be explicitly acknowledged at install (the existing exact-match ack gate enforces it). The operator's own apps never go through install, so this only gates third-party bundles (single-operator framing). Test in `apps.test.ts`: a bundle with a python `code` node surfaces `executes-code:python`; partial ack → 403; full ack → 201. App-package suite 13/13 green. _(Note: API `tsc` currently reports 4 `Buffer`/`BlobPart` errors in the channel adapters from concurrent in-flight work — unrelated to this; app package typechecks clean.)_
- _2026-06-24_ — **Phase 2.1 COMPLETE: native SaaS webhooks turned on.** The 14 provider verifiers in `triggerConnectors.ts` (github/slack/linear/stripe/typeform/gmail/shopify/hubspot/intercom/zendesk/twilio/discord/pagerduty/sendgrid) were fully built + unit-tested but imported by nothing — the platform was poll-only. New `TriggerRuntime.fireConnectorWebhook` resolves the provider from the trigger config (`connectorFromConfig`), verifies the provider's own signature with the trigger's stored `webhookSecret`, and fires through a shared `#deliverWebhook` (idempotency + dispatch, extracted from `fireWebhook` so both paths share it). New route `POST /v1/webhooks/connector/:triggerId`. Test `TriggerRuntime.connectorWebhook.test.ts` (3): valid GitHub sig → fires once + idempotent replay; forged sig → `WEBHOOK_SIGNATURE_INVALID`; generic (no connector) → refused. Trigger/webhook suites green (no regression from the `#deliverWebhook` extraction). Typecheck clean. **"when a Stripe payment fails, do X" is now reachable.**
- _2026-06-24_ — **Phase 1.7-partial: `human_input` form node SHIPPED.** New `human_input` node kind (core `HumanInputNodeConfig` w/ `fields[]` + `outputKey?`, `WorkflowNodeType`/union, `SUPPORTED_NODE_KINDS`) pauses the run for a human to submit structured form values that become the node's output — "draft → human fills subject/send-date → continue". Reuses the checkpoint approval/resume path (no new pending-kind): threaded submitted `data` end-to-end through `ApprovalInboxService.resolve` → `CheckpointResumeHandler` → bootstrap → `engine.resolveApproval`, which detects a human_input target and completes it with the values (wrapped under `outputKey` when set); reject fails the node so the run can't hang. The pause event carries the form spec for the UI. Test `WorkflowEngine.humanInput.test.ts` (3): submit→values-as-output, outputKey wrapping, reject→FAILED. Approval/checkpoint/humanGate suites green (no regression). Core+API typecheck clean.
- _2026-06-24_ — **Phase 1.7-partial: `wait` until an absolute datetime SHIPPED.** `WaitNodeConfig.untilIso?` (ISO 8601, template-resolved upstream); `#executeWait` waits until that instant (past = instant, invalid = `VALIDATION_FAILED`) instead of only a relative `delayMs`. No zod change (wait flows through the permissive fallback config schema). Enables "send Monday 9am" / SLA schedules. Test `WorkflowEngine.waitUntil.test.ts` (3): future waits ≥150ms→COMPLETED, past→instant COMPLETED, invalid→FAILED. The aggregation-window / pagination / human-input-form pieces of 1.7 remain.
- _2026-06-24_ — **Phase 1.3 (generic per-node `retryPolicy`) + 1.6 (explicit merge↔parallel binding) SHIPPED** (schema-gated items, unblocked now the in-flight refactor is committed). (1.3) New `NodeRetryPolicy` on the base `WorkflowNode` (`{ maxAttempts, backoffMs?, retryOn? }`) + zod; the engine applies it centrally in `#failNode` BEFORE error-edge routing — a transient IO/deterministic failure re-dispatches the node with exponential backoff (capped 60s, ≤10 attempts), holding the run open via a synthetic execution; agent-like kinds excluded (they own self-heal). Test `WorkflowEngine.retryPolicy.test.ts` (3): a mock connector that fails-once-then-succeeds → COMPLETED in 2 calls/1 retry; always-fails → 3 calls/2 retries/FAILED; no-policy → 1 call/0 retries/FAILED. (1.6) New optional `merge.parallelSourceId` + zod; `#resolveJoinPolicy` now reads the join policy from the *named* parallel via `#parallelForMerge`, falling back to the nearest-upstream heuristic when unset/unresolvable. Test `WorkflowEngine.mergeBinding.test.ts` (2): a two-parallel diamond where nearest=P1 (merge_keys) but `parallelSourceId='P2'` yields P2's collect_all output — proving the binding overrides the heuristic. Core+API typecheck clean; failure-path engine suite green (no regression).
